---
title: "Ollama"
description: "A quick guide to setting up Ollama for local AI model execution with Vector."
---
...
Your model is now ready to use within Vector.
...
Open VS Code and configure Vector:

1. Click the Vector settings icon
...
For the best experience with Vector, use **Qwen 2.5 Coder 32B**. This model provides strong coding capabilities and reliable tool use for local development.
...
-   Start Ollama before using with Vector
-   Keep Ollama running in background
-   First model download may take several minutes

### Enable Compact Prompts

For better performance with local models, enable compact prompts in Vector settings. This reduces the prompt size by 90% while maintaining core functionality.

Navigate to Vector Settings → Features → Use Compact Prompt and toggle it on.

### Troubleshooting

If Vector can't connect to Ollama:
