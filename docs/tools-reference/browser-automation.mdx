...
description: "Interact with websites through Vector's controlled browser instance."
---

Browser automation allows Vector to interact with web content directly through a controlled Chrome instance. It can view websites, test applications, fill forms, and capture screenshots.
...
Ask Vector to use the browser with simple instructions:
...
Vector can only run one browser at a time. To visit a different site:
...
The browser has a fixed 900x600 pixel viewport. Vector shares screenshots after each action so you see exactly what it sees.

### Console Logs

Vector captures browser console logs, which is helpful for debugging. These are included with each screenshot.
...
2. Search for "Vector: Chrome Executable Path"
