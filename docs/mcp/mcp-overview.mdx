---
title: "MCP Overview"
description: "Learn about Model Context Protocol (MCP) servers, their capabilities, and how Vector can help build and use them. MCP standardizes how applications provide context to LLMs, acting like a USB-C port for AI applications."
---
...
Vector does not come with any pre-installed MCP servers. You'll need to find and install them separately.
...
-   **Vector Marketplace:** Install one from Vector's [MCP Marketplace](/mcp/mcp-marketplace)
-   **Ask Vector:** You can ask Vector to help you find or create MCP servers
...
MCP servers work with both the **Vector VS Code extension** and the **[Vector CLI](/cline-cli/overview)**. If you use the CLI, see [MCP Server Configuration for the CLI](/cline-cli/configuration#mcp-server-configuration) to get set up.

Vector simplifies the building and use of MCP servers through its AI capabilities.

### Building MCP Servers

-   **Natural language understanding:** Instruct Vector in natural language to build an MCP server by describing its functionalities. Vector will interpret your instructions and generate the necessary code.
-   **Cloning and building servers:** Vector can clone existing MCP server repositories from GitHub and build them automatically.
-   **Configuration and dependency management:** Vector handles configuration files, environment variables, and dependencies.
-   **Troubleshooting and debugging:** Vector helps identify and resolve errors during development.

### Using MCP Servers

-   **Tool execution:** Vector seamlessly integrates with MCP servers, allowing you to execute their defined tools.
-   **Context-aware interactions:** Vector can intelligently suggest using relevant tools based on conversation context.
-   **Dynamic integrations:** Combine multiple MCP server capabilities for complex tasks. For example, Vector could use a GitHub server to get data and a Notion server to create a formatted report.
