...
description: "Learn about the two primary transport mechanisms for communication between Vector and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Vector and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Vector) spawns an MCP server as a child process
...
-   **Locality**: Runs on the same machine as Vector
...
-   Single-client scenarios (one Vector instance per server)
...
1. The client (Vector) connects to the server's SSE endpoint via HTTP GET request
...
-   **Remote Access**: Can be hosted on a different machine from your Vector instance
...
STDIO servers run locally on the same machine as Vector, which has several important implications:
...
-   **Execution**: Starts and stops with Vector (child process lifecycle)
...
-   Start when needed by Vector
-   Not require network configuration
-   Need to be installed alongside Vector or via a package manager
...
## Configuring Transports in Vector

For detailed information on configuring STDIO and SSE transports in Vector, including examples, see [Configuring MCP Servers](/mcp/configuring-mcp-servers).
