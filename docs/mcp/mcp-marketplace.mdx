---
title: "MCP Made Easy"
description: "Learn how to use the MCP Marketplace to discover, install, and configure MCP servers that enhance Vector's capabilities with additional tools and resources."
---

## What's an MCP Server?

MCP servers are specialized extensions that enhance Vector's capabilities. They enable Vector to perform additional tasks like fetching web pages, processing images, accessing APIs, and much more.
...
-   In Vector, click the "Extensions" button (square icon) in the top toolbar
...
-   If the server requires an API key (most do), Vector will guide you through:
...
-   Vector will show confirmation when installation is complete
-   Check the server status in Vector's MCP settings UI

### 5. Using Your New Server

-   After successful installation, Vector will automatically integrate the server's capabilities
-   You'll see new tools and resources available in Vector's system prompt
-   Simply ask Vector to use the capabilities of your new server
-   Example: "Search the web for recent React updates using Perplexity"

**Corporate Users:** If you're using Vector in a corporate environment, ensure you have permission to install third-party MCP servers according to your organization's security policies.
...
-   The server code is cloned/installed to `/Users/<username>/Documents/Vector/MCP/`
...
-   Vector detects the configuration change
-   Vector launches your server as a separate process
-   Communication is established via stdio or HTTP

### 4. Integration with Vector

-   Your server's capabilities are added to Vector's system prompt
-   Tools become available via `use_mcp_tool` commands
-   Resources become available via `access_mcp_resource` commands
-   Vector can now use these capabilities when prompted by the user
...
-   Try removing and reinstalling the server (remove from both the `cline_mcp_settings.json` file and the `/Users/<username>/Documents/Vector/MCP/` directory)
-   Use a terminal and run the command with its arguments directly. This will allow you to see the same errors that Vector is seeing

## MCP Server Rules

Vector is already aware of your active MCP servers and what they are for, but when you have a lot of MCP servers enabled, it can be useful to define when to use each server.

Utilize a `.clinerules` file or custom instructions to support intelligent MCP server activation through keyword-based triggers, making Vector's tool selection more intuitive and context-aware.

### How MCP Rules Work

MCP Rules group your connected MCP servers into functional categories and define trigger keywords that activate them automatically when detected in your conversations with Vector.
...
-   **Contextual Tool Selection**: Vector selects appropriate tools based on conversation context
...
When you write "Can you scrape this website?", Vector detects "scrape" and "website" as triggers, automatically selecting web-related MCP servers.
...
Add this to your `.clinerules` file or to your custom instructions to make Vector's MCP server selection more intuitive and context-aware.
