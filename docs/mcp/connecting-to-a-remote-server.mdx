...
description: "The Model Context Protocol (MCP) allows Vector to communicate with external servers that provide additional tools and resources to extend its capabilities. This guide explains how to add and connect to remote MCP servers through the MCP Servers interface."
---
...
To access the MCP Servers interface in Vector:

1. Click on the Vector icon in the VSCode sidebar
2. Open the menu (⋮) in the top right corner of the Vector panel
...
4. Vector will attempt to connect to the server and display the connection status

> **Note**: When connecting to a remote server, ensure you trust the source, as MCP servers can execute code in your environment.

#### Transport Types

Vector supports two transport protocols for remote MCP servers:
...
6. You'll be redirected back to Vector automatically
7. The server will connect and show a green status dot

Once authenticated, your credentials are securely stored and the server will reconnect automatically when you reload Vector. You won't need to authenticate again unless you delete the server or your credentials expire.
...
> **Warning**: The following third-party marketplaces are listed for informational purposes only. Vector does not endorse, verify, or take responsibility for any servers listed on these marketplaces. These servers are cloud-hosted services that process your requests and may have access to data you share with them. Always review privacy policies and terms of use before connecting to third-party services.
...
2. **Request Timeout**:
    - Set how long Vector should wait for server responses
...
-   **Enabled**: Vector can use the server's tools and resources
...
-   **Redirect errors**: Verify you're using the latest version of Vector - older versions may not support OAuth
-   **Reset authentication**: Delete the server and re-add it to start fresh with a new OAuth flow

### Advanced Configuration

For advanced users, Vector stores MCP server configurations in a JSON file that can be modified:
...
Once connected, Vector can use the tools and resources provided by the MCP server. When Vector suggests using an MCP tool:

1. A tool approval prompt will appear (unless auto-approved)
2. Review the tool details and parameters before approving
3. The tool will execute and return results to Vector
...
5. Sign in to Vercel in your browser and authorize Vector
6. Return to Vector - the server will automatically connect
7. Vercel's tools (deploy, logs, projects) are now available to Vector!

Your Vercel authentication persists across sessions, so you won't need to re-authenticate each time you use Vector.
