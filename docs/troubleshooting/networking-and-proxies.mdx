...
description: "Configure Vector to work behind firewalls and proxies"
---

If you're working behind a corporate proxy or firewall, you'll need to configure
proxy settings for Vector to connect to AI providers. The configuration varies
depending on which version of Vector you're using.
...
is needed for Vector itself.

## CLI

The Vector CLI uses standard HTTP proxy environment variables. Configure these before running `cline` commands.
...
Vector CLI only supports HTTP proxies. It does not support SOCKS proxies,
...
After configuring the proxy, test that Vector can connect to your AI provider:

1. Open the Vector panel
...
Vector in JetBrains only supports HTTP proxies. It does not support SOCKS
proxies, proxy autoconfiguration (PAC) scripts, or HTTP proxies which require
authentication beyond a basic username and password.

Vector does not pick up changed proxy settings dynamically. After changing proxy
settings, restart the IDE for Vector to use the new settings.
...
If you encounter problems with Vector networking, first verify your proxy
