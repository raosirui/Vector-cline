...
description: "How to write and contribute to Vector documentation"
---

Vector's documentation lives in the `docs/` directory and uses [Mintlify](https://mintlify.com) for rendering. This guide covers how to write docs that match Vector's established style.

## Using the Documentation Workflow

The fastest way to create documentation is using the `/write-docs` workflow. Type `/write-docs` in Vector and describe what you want to document. Vector guides you through a 4-step process:
...
```markdown
# Good
Switch to bash in Vector Settings → Terminal → Default Terminal Profile.

# Bad
Users who are experiencing issues may find it helpful to navigate to the 
Vector settings menu where they can locate the terminal configuration 
...
parts of the codebase. For example, when adding authentication, Vector 
...
Vector does things. Files don't get created by Vector, Vector creates files.

```markdown
# Good
Vector reads your project files and builds context automatically.

# Bad
Project files are read and context is built automatically.
```

### Use Neutral Pronouns for Vector

Refer to Vector as "it" not "he". Vector is software, not a person.

```markdown
# Good
When Vector encounters an error, it suggests fixes.

# Bad
When Vector encounters an error, he suggests fixes.
```
...
<Steps>
  <Step title="Install the Extension">
    Search for "Vector" in the VS Code marketplace.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card title="Workflows" icon="diagram-project" href="/customization/workflows">
    Learn about Vector's workflow system.
  </Card>
</CardGroup>
