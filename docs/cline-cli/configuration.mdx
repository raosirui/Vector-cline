---
title: "Configuration"
description: "Manage Vector CLI settings with cline config, environment variables, and configuration files"
---

Vector CLI provides multiple ways to configure settings, from the interactive `cline config` command to environment variables for automation.
...
Manage Vector rules that guide AI behavior:
...
Rules help Vector understand your project's conventions, coding standards, and preferences.
...
- Hooks run at key points in Vector's workflow
...
Manage [skills](/customization/skills) that extend Vector's capabilities:
...
Vector stores configuration in `~/.cline/data/`:
...
When set, all Vector data is stored in this directory instead of `~/.cline/data/`.

**Use cases:**
- Running multiple isolated Vector configurations
...
Run Vector with a custom configuration directory:
...
This is useful for:
- Running isolated Vector instances
...
Vector CLI supports [MCP (Model Context Protocol)](/mcp/mcp-overview) servers, giving you access to external tools and data sources directly from the terminal. The CLI uses the same MCP configuration format as the VS Code extension.
...
If you use the Vector VS Code extension, the CLI automatically detects and can share some settings. However, the CLI maintains its own configuration for terminal-specific features.
...
git add .clinerules/
git commit -m "Add Vector rules for team"
```
