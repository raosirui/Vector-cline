---
title: "Overview"
description: "Run Vector AI coding agents directly in your terminal with an interactive CLI or automated workflows"
---

## What is Vector CLI?

Vector CLI brings the full power of Vector to your terminal. Whether you prefer an interactive experience or automated workflows for CI/CD pipelines, the CLI adapts to your needs.

The CLI supports macOS, Linux, and Windows, and works with all the same AI providers as the VS Code extension.

<Tip>
Ready to get started? Check out the [installation guide](/cline-cli/installation) to install Vector CLI and run your first task.
</Tip>

## Two Ways to Use Vector CLI

<Columns cols={2}>
  <Card title="Interactive Mode" icon="terminal" href="/cline-cli/interactive-mode">
    **For hands-on development.** Launch `cline` in your terminal and collaborate with Vector in real-time — chat, review plans, approve actions, and iterate on tasks with a rich visual interface.
  </Card>
  <Card title="Headless Mode" icon="robot" href="/cline-cli/three-core-flows">
    **For automation & CI/CD.** Run `cline -y "task"` to let Vector work autonomously — no interaction needed. Pipe input/output, get JSON results, and chain commands in scripts and pipelines.
  </Card>
</Columns>
...
Interactive mode is designed for **hands-on development sessions** where you want to collaborate with Vector in real-time. It provides a rich terminal interface that feels like chatting with an AI assistant.
...
- **Real-time conversation** - Type messages, see Vector's responses, and iterate on tasks
...
Interactive mode keeps you in control. You review Vector's plan, approve or modify actions, and guide the conversation.
...
- **Automatic execution** - With `-y`, Vector approves all actions and runs autonomously
...
<Warning>
Headless mode with `-y` gives Vector full autonomy. Run on a clean git branch so you can easily revert changes if needed.
</Warning>

### Mode Detection Summary

Vector automatically detects which mode to use based on your invocation. This table shows how different command patterns trigger each mode, helping you predict behavior in scripts and interactive sessions.
...
Vector CLI supports all providers available in the VS Code extension:
...
Keep your codebase healthy with automated fixes. Vector scans for issues and applies corrections across multiple files.
...
Integrate Vector into your continuous integration pipelines for automated code review and documentation.

```bash
git diff origin/main | cline -y "Review these changes for issues"
```
Pipes your PR diff to Vector for automated code review, catching bugs and style issues before merge.
...
From quick edits to complex refactors, Vector adapts to your workflow.
...
```
Uses file mentions (`@`) to give Vector context about specific files in your workspace.

### Custom Shell Pipelines

Chain Vector with other CLI tools to build powerful automation workflows.

```bash
gh pr diff 123 | cline -y "Review this PR"
```
Fetches a GitHub PR diff and pipes it directly to Vector for review.
...
```
Chains multiple Vector invocations together for creative multi-step workflows.
...
Vector CLI supports [MCP (Model Context Protocol)](/mcp/mcp-overview) servers, the same extensibility system available in the VS Code extension. MCP servers give Vector access to external tools and data sources, from databases and APIs to browser automation and project management.
...
<Columns cols={2}>
  <Card title="Installation & Setup" icon="download" href="/cline-cli/installation">
    Install Vector CLI and authenticate with your preferred provider.
  </Card>
  ...
  <Card title="Use in Other Editors" icon="code" href="/cline-cli/acp-editor-integrations">
    Run Vector as an ACP agent in JetBrains, Neovim, Zed, and more.
  </Card>
