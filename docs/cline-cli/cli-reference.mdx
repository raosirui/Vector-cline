---
title: "CLI Reference"
description: "Complete command reference for Vector CLI including all commands, flags, and configuration options"
---

This page documents all available commands, flags, and configuration options for Vector CLI. For quick help in your terminal, use:
...
Vector CLI automatically detects the best output mode based on how you invoke it:
...
Vector operates in two primary modes that control how it approaches tasks:

| Mode | Description |
|------|-------------|
| **Act Mode** (default) | Vector actively uses tools to accomplish tasks. It can read files, write code, execute commands, use a headless browser, and more. |
| **Plan Mode** | Vector gathers information and creates a detailed plan before implementation. It explores the codebase, asks clarifying questions, and presents a strategy for your approval before switching to Act Mode. |
...
Run Vector without a subcommand to start a task or enter interactive mode.
...
| Option | Description |
|--------|-------------|
| `-a, --act` | Start in Act mode (default). Vector executes actions directly. |
| `-p, --plan` | Start in Plan mode. Vector analyzes and creates a strategy before acting. |
...
When set, all Vector data (settings, secrets, task history) is stored in this directory instead of `~/.cline/data/`.

**Use cases:**
- Running isolated Vector instances with different settings
- CI/CD environments with custom state directories
- Testing configuration changes without affecting your main setup

### CLINE_COMMAND_PERMISSIONS

Restrict which shell commands Vector can execute:
...
Vector stores all data in `~/.cline/` by default:
...
<Columns cols={2}>
  <Card title="Installation & Setup" icon="download" href="/cline-cli/installation">
    Install Vector CLI and configure authentication.
  </Card>
