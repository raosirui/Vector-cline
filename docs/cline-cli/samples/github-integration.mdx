...
description: "Automatically respond to GitHub issues by mentioning @cline in comments using Vector CLI in GitHub Actions."
---
...
<Note>
**New to Vector CLI?** This sample assumes you understand Vector CLI basics and have completed the [Installation Guide](https://docs.cline.bot/cline-cli/installation). If you're new to Vector CLI, we recommend starting with the [GitHub RCA sample](./github-issue-rca) first, as it's simpler and will help you understand the fundamentals before setting up GitHub Actions.
</Note>

## The Workflow

Trigger Vector by mentioning `@cline` in any issue comment:

<Frame>
  <img src="https://storage.googleapis.com/cline_public_images/ss0a-comment.png" alt="Issue comment with @cline mention" width="600" />
</Frame>

Vector's automated analysis appears as a new comment, with insights drawn from your actual codebase:

<Frame>
  <img src="https://storage.googleapis.com/cline_public_images/ss0b-final.png" alt="Automated analysis response from Vector" width="600" />
</Frame>
...
- **Vector CLI knowledge** - Completed the [Installation Guide](https://docs.cline.bot/cline-cli/installation) and understand basic usage
...
jobs:
  respond:
    ...
    steps:
      ...
      # Node v20+ is needed for Vector CLI on GitHub Actions Linux
      - name: Setup Node.js
      ...
      - name: Configure Vector Authentication
        if: steps.detect.outputs.hit == 'true'
        env:
...
start up the Vector CLI to dig into the issue, providing feedback as a reply to the issue.
...
Now you're ready to supply Vector with the credentials it needs in a GitHub Action.
...
```bash
#!/bin/bash
# Analyze a GitHub issue using Vector CLI
...
# Ask Vector for its analysis, showing only the summary
...
This analysis script calls Vector to execute a prompt on a GitHub issue,
...
git commit -m "Add Vector issue assistant workflow"
git push
```
...
3. **Installs** Vector CLI globally using npm
