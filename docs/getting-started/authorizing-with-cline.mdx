...
description: "Authenticate with Vector and choose your first AI model"
---

Vector connects to AI models through a **provider**. You have two paths:

<CardGroup cols={2}>
  <Card title="Vector Provider" icon="bolt">
...
<Steps>
  <Step title="Open Settings">
    Click the settings icon in the top-right of the Vector panel.
  </Step>

  <Step title="Select a Provider">
    Choose from the **API Provider** dropdown:
    - **Vector** — simplest setup, no API key needed
  ...
  <Step title="Authenticate">
    **Vector Provider:** Click **Sign In** and authenticate via Google, GitHub, or email. See [OAuth details](#how-oauth-works) below.
    ...
    **Local:** No key needed — just ensure your local server is running.
    ...
    <Note>
    API keys are stored in your system's credential manager and sent only to your selected provider. They are never logged or transmitted to Vector's servers.
    </Note>
  </Step>
  ...
  <Step title="Verify">
    Send any message. If Vector responds, you're ready.
  </Step>
</Steps>

## Vector Provider

The Vector Provider gives you one account, one billing relationship, and access to models from Anthropic, OpenAI, Google, and more.
...
Click **Add Credits** in Vector settings or visit your [account dashboard](https://app.cline.bot/dashboard). Credits work across all available models.

### How OAuth Works

<Steps>
  <Step title="Sign In">
    Click **Sign In** in Vector settings. Your browser opens to `app.cline.bot`.
  </Step>
  <Step title="Authenticate">
...
- **Balance & usage:** Open Vector settings — your credit balance is at the top. Click **View Usage** for transaction history.
- **Switch organization:** Go to Vector settings → **Switch Organization** to change which billing account is charged.
...
| Browser doesn't open | Check default browser settings. Copy the URL from the Vector output panel manually. |
...
- [Your First Project](/getting-started/your-first-project) — build something with Vector
- [Core Workflows](/core-workflows/task-management) — patterns you'll use daily
- [Customization](/customization/overview) — tailor Vector to your workflow
