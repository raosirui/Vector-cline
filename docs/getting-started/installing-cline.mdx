---
title: "Installing Vector"
description: "Get Vector up and running in your favorite IDE or terminal with these simple installation steps"
---
...
<Tabs>
  <Tab title="VS Code/Cursor/Antigravity">
    <Steps>
      ...
      <Step title="Search for Vector">
        Type "Vector" in the search bar.
      </Step>
      <Step title="Install">
        Click the **Install** button on the Vector extension.
      </Step>
      <Step title="Access Vector">
        Click the Vector icon in the Activity Bar, or open Command Palette (`Ctrl/Cmd + Shift + P`) and type "Vector: Open In New Tab".
      </Step>
    </Steps>

    <Tip>
    If VS Code shows "Running extensions might..." dialog, click **Allow**. If you don't see the Vector icon, restart VS Code.
    </Tip>
  </Tab>
  <Tab title="JetBrains IDEs">
    <Note>
    Vector for JetBrains works almost identically to VS Code, with all core features: diff editing, tools, multiple API providers, MCP servers, Vector rules/workflows, and more.
    </Note>
    ...
    <Steps>
      ...
      <Step title="Install Vector">
        Search for "Vector" and click **Install**.
      </Step>
    ...
    Find Vector in **View** → **Tool Windows** → **Vector** (usually on the right side).
  ...
  <Tab title="CLI">
    <Warning>
    **Preview Release**: Vector CLI is currently in preview and only available for macOS and Linux. Windows support is coming soon.
    </Warning>

    Vector CLI runs AI coding agents directly in your terminal. Use it for automated code reviews in CI/CD, multi-instance development, or shell workflow integration.

    <Steps>
      <Step title="Install Node.js 20+">
        Check your version with `node --version`. If needed, visit [nodejs.org](https://nodejs.org) or use nvm.
      </Step>
      <Step title="Install Vector CLI">
        Run `npm install -g cline` in your terminal.
      </Step>
      <Step title="Authenticate">
        Run `cline auth` to sign in and configure your AI model provider.
      </Step>
      <Step title="Run Vector">
        Run `cline` to start an interactive session, or `cline "Your task here"` for headless execution.
      </Step>
    </Steps>

    <Tip>
    Want to learn more? See the [Vector CLI documentation](/cline-cli/getting-started) for advanced usage patterns like multi-instance development and CI/CD integration.
    </Tip>
  </Tab>
  <Tab title="Zed/Neovim (ACP via CLI)">
    <Note>
    **ACP (Agent Client Protocol)** lets you run Vector in any ACP-compatible editor via the CLI. This gives you full access to Vector's capabilities—including Skills, Hooks, and MCP integrations—in your preferred editor.
    </Note>
    ...
    <Steps>
      ...
      <Step title="Configure your editor">
        <Tabs>
          <Tab title="Zed">
            Open Zed settings (`Cmd/Ctrl + ,`) and add Vector to your `settings.json`:
            ```json
            {
              "agent_servers": {
                "Vector": {
            ...
            ```
            Then open the AI assistant panel, select **Vector** from the agent dropdown, and start coding.
          </Tab>
          <Tab title="Neovim (agentic.nvim)">
            ...
            ```lua
            {
              ...
              keys = {
                {"<C-\\>", function() require("agentic").toggle() end, mode={"n","v","i"}, desc="Toggle Vector Chat"},
              },
            }
            ```
            Press `<C-\>` to toggle the Vector chat panel.
          </Tab>
  ...
  <Tab title="VSCodium/Windsurf">
    ...
    <Steps>
      ...
      <Step title="Install">
        Select "Vector" by saoudrizwan and click **Install**.
      </Step>
      <Step title="Reload">
    ...
    Look for the Vector icon in your Activity Bar or use the Command Palette.
  </Tab>
</Tabs>
...
1. **Open Vector** in your editor:
   - **VS Code/Cursor/Antigravity/VSCodium/Windsurf:** Click the Vector icon in the Activity Bar
   - **JetBrains:** Go to **View** → **Tool Windows** → **Vector**

2. **Sign in** by clicking the **Sign Up** button in the Vector interface. You'll be redirected to [app.cline.bot](https://app.cline.bot) to authenticate.

   <Tip>
     Learn more about [authorizing with Vector](/getting-started/authorizing-with-cline), including how OAuth authentication works, using API keys with other providers, and troubleshooting auth issues.
   </Tip>

3. **Start building!** After signing in, you'll automatically return to your editor-Vector is ready to help.

## Setting Up Vector in the Right Sidebar

For the best coding experience, we recommend moving Vector to the right sidebar. This keeps your project files visible on the left while you chat with Vector on the right, giving you full visibility of your codebase as Vector works.
...
<Tabs>
  <Tab title="VS Code">
    <Steps>
      ...
      <Step title="Drag Vector Icon">
        Drag the Vector icon over to the nav panel at the top of that right view.
      </Step>
    </Steps>
  ...
  <Tab title="Cursor">
    <Steps>
      ...
      <Step title="Drag Vector to the AI Pane">
        Drag the Vector icon directly into the AI Pane sidebar.
      </Step>
    </Steps>
...
### Can't Find Vector in the Marketplace

Sometimes Vector doesn't show up in search results if you're looking in the wrong tab or using an incompatible IDE version. Make sure you're searching in the **Marketplace** tab (not Installed), try searching for "Vector AI" instead, and verify your IDE is up to date. If installation fails, restart your IDE and check your internet connection.

### Vector Icon Not Appearing After Install

The most common fix is a full restart-close your IDE completely (File → Exit) and reopen it. In VS Code/Cursor/VSCodium, you can also open the Command Palette (`Ctrl/Cmd + Shift + P`) and type "Vector: Open In New Tab". In JetBrains, check **View** → **Tool Windows** → **Vector**. If it's still missing, verify the plugin is enabled in your Extensions/Plugins settings.

### CLI: Node.js or Permission Errors

Vector CLI requires Node.js 20 or higher. Run `node --version` to check-if you need to upgrade, use nvm (`nvm install 22 && nvm use 22`) or download from [nodejs.org](https://nodejs.org). For permission errors on `npm install -g`, either prefix with `sudo` on macOS/Linux or configure npm to use a user-owned directory for global packages.

### Plugin Installed But Not Working

If Vector appears installed but doesn't respond, try disabling and re-enabling the extension in your IDE's settings. Check the Developer Console (VS Code: Help → Toggle Developer Tools) or Event Log (JetBrains) for error messages. Also ensure you're using a supported IDE version and close any resource-intensive extensions that might interfere.
