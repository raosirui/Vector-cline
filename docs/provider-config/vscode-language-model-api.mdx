...
description: "Learn how to use Vector with the experimental VS Code Language Model API, enabling access to models from GitHub Copilot and other compatible extensions."
---

Vector offers _experimental_ support for the [VS Code Language Model API](https://code.visualstudio.com/api/extension-guides/language-model). This API enables extensions to grant access to language models directly within the VS Code environment. Consequently, you might be able to leverage models from:
...
1. **Ensure Copilot Account is Active and Extensions are installed:** User logged into either the Copilot or Copilot Chat extension should be able to gain access via Vector.
2. **Access Vector Settings:** Click the gear icon (⚙️) located in the Vector panel.
...
-   **Dependency on Extensions:** This feature is entirely contingent on other extensions making models available. Vector does not directly control the list of accessible models.
...
-   **No Direct Cost Management:** Users are subject to the pricing structures and terms of service of the extension providing the model. Vector cannot directly monitor or regulate associated costs.
-   **GitHub Copilot Rate Throttling:** When employing the VS Code LM API with GitHub Copilot, be mindful that GitHub may enforce rate limits on Copilot usage. These limitations are governed by GitHub, not Vector.
