---
title: "Mistral"
description: "Learn how to configure and use Mistral AI models, including Codestral, with Vector. Covers API key setup and model selection."
---

Vector supports accessing models through the Mistral AI API, including both standard Mistral models and the code-specialized Codestral model.
...
Vector supports the following Mistral models:
...
To use Codestral with Vector:

1.  **Select "Mistral" as the API Provider in Vector Settings.**
2.  **Select a Codestral Model** (e.g., `codestral-latest`) from the "Model" dropdown.
3.  **Enter your Codestral API Key** (from `codestral.mistral.ai`) or your La Plateforme API Key (from `api.mistral.ai`) into the appropriate API key field in Vector.
