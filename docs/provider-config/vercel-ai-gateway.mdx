...
description: "Use Vercel AI Gateway in Vector to reach 100+ models from one endpoint with routing, retries, and spend observability."
---

Vercel AI Gateway gives you a single API to access models from many providers. You switch by model id without swapping SDKs or juggling multiple keys. Vector integrates directly so you can pick a Gateway model in the dropdown, use it like any other provider, and see token and cache usage in the stream.
...
4. Pick a model from the list. Vector fetches the catalog automatically. You can also paste an exact id
...
The gateway supports a large and changing set of models. Vector pulls the list from the Gateway API and caches it locally. For the current catalog, see https://vercel.com/ai-gateway/models
...
- Multi-model evals - swap only the model id in Vector and compare latency and output tokens
