...
description: "Set up AWS Bedrock with Vector using IAM Access Key and Secret Key credentials. Best for enterprise environments with established IAM policies."
---
...
2.  Use the JSON editor to add the following policy document:
    ...
    ```
3.  Name the policy (e.g., `VectorBedrockInvokeAccess`) and attach it to your IAM user or role.
...
By following these steps, your enterprise team can securely integrate AWS Bedrock with the Vector VS Code extension to accelerate development:
...
3. **Configure Vector in VS Code:** Install and set up Vector with your AWS credentials and choose an appropriate model.
