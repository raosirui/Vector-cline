...
description: "Set up AWS Bedrock with Vector using Bedrock API Keys. Simplest setup for individual developers to access frontier models."
---
...
-   **Vector:** A VS Code extension that acts as a coding assistant by integrating with AI models-empowering developers to generate code, debug, and analyze data.
...
To ensure Vector can interact with AWS Bedrock, your IAM user or role needs specific permissions. While the `AmazonBedrockLimitedAccess` managed policy provides comprehensive access, for a more restricted and secure setup adhering to the principle of least privilege, the following minimal permissions are sufficient for Vector's core model invocation functionality:
...
2.  Use the JSON editor to add the following policy document:
    ...
    ```
3.  Name the policy (e.g., `VectorBedrockInvokeAccess`) and attach it to the IAM user associated with the key you created. The IAM user and the API key have the same prefix.

**Important Considerations:**

-   **Model Listing in Vector:** The minimal permissions (`bedrock:InvokeModel`, `bedrock:InvokeModelWithResponseStream`) are sufficient for Vector to _use_ a model if you specify the model ID directly in Vector's settings. If you rely on Vector to dynamically list available Bedrock models, you might need additional permissions like `bedrock:ListFoundationModels`.
-   **AWS Marketplace Subscriptions:** For third-party models (e.g., Anthropic Claude), the **`AmazonBedrockLimitedAccess`** policy grants you the necessary permissions to subscribe via the AWS Marketplace. There is no explicit access to be enabled. For Anthropic models you are still required to submit a First Time Use (FTU) form via the Console. If you get the following message in the Vector chat `[ERROR] Failed to process response: Model use case details have not been submitted for this account. Fill out the Anthropic use case details form before using the model.` then open the [Playground in the AWS Bedrock Console](https://console.aws.amazon.com/bedrock/home?#/text-generation-playground), select any Anthropic model and fill in the form (you might need to send a prompt first)
...
### Step 3: Configure the Vector VS Code Extension

#### 3.1 Install and Open Vector
...
2. **Install the Vector Extension:**
    ...
    - Search for **Vector** and install it.

#### 3.2 Configure Vector Settings

1. **Open Vector Settings:**
...
By following these steps, you can quickly integrate AWS Bedrock with the Vector VS Code extension to accelerate development:
...
3. **Configure Vector in VS Code:** Install and set up Vector with your AWS API Key and choose an appropriate model.
...
_This guide will be updated as AWS Bedrock and Vector evolve. Always refer to the latest documentation and internal policies for up-to-date practices._
