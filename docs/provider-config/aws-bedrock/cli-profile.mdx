...
description: "Configure AWS Bedrock to use AWS CLI profiles for authentication with Vector. Best for SSO/federated roles and secure enterprise access."
---

### Overview

Vector offers the option of utilizing AWS credentials or AWS profiles to access AWS Bedrock services. SSO/Federated roles are suggested over Legacy IAM configuration; this guide describes how to configure your environment so that Vector uses SSO roles for authentication.
...
3. Continue configuring your profile using [the `aws configure sso` CLI wizard](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-sso.html#cli-configure-sso-configure)
    ...
    - Note which profile name you attach to your AWS account, this is needed to configure Vector in the following steps

4. If you haven't already done so, install VSCode and the Vector extension. Consult the [Getting Started](/getting-started/installing-cline) page for guidance.

5. Open the Vector extension, then click on the settings button ⚙️ to select your API Provider.
...
<Frame>
	<img
		...
		alt="AWS Bedrock configuration in Vector settings showing profile authentication setup"
	/>
</Frame>
