...
description: "Learn how to configure Vector with various AI model providers that offer OpenAI-compatible APIs."
---

Vector supports a wide range of AI model providers that offer APIs compatible with the OpenAI API standard. This allows you to use models from providers _other than_ OpenAI, while still utilizing a familiar API interface. This includes providers such as:
...
The key to using an OpenAI-compatible provider with Vector is to configure these main settings:
...
You'll find these settings in the Vector settings panel (click the ⚙️ icon):
...
While the "OpenAI Compatible" provider type allows connecting to various endpoints, if you are connecting directly to the official OpenAI API (or an endpoint that mirrors it exactly), Vector recognizes the following model IDs based on the `openAiNativeModels` definition in its source code:
...
By using an OpenAI-compatible provider, you can leverage the flexibility of Vector with a wider array of AI models. Remember to always consult your provider's documentation for the most accurate and up-to-date information.
