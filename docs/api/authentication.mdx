...
description: "How to authenticate with the Vector API using API keys or account tokens."
---

Every request to the Vector API requires authentication via a Bearer token in the `Authorization` header.
...
| **Account auth token** | Vector extension and CLI | Generated automatically when you sign in |
...
When you sign in to the Vector extension (VS Code, JetBrains) or CLI, an account auth token is generated and managed automatically. You do not need to handle these tokens manually.

The Vector CLI uses these tokens when you authenticate via:
...
The Vector API accepts optional headers for tracking and identification:

| Header | Description |
|--------|-------------|
| `HTTP-Referer` | Your application's URL. Helps with usage tracking. |
| `X-Title` | Your application's name. Appears in usage logs. |
| `X-Task-ID` | A unique task identifier. Used internally by the Vector extension. |
