...
description: "Reference for the Vector Chat Completions API, an OpenAI-compatible endpoint for programmatic access."
---

The Vector API provides an OpenAI-compatible Chat Completions endpoint. You can use it from the Vector extension, the CLI, or any HTTP client that speaks the OpenAI format.
...
- **Account auth token** (used automatically by the Vector extension and CLI when you sign in)
...
The easiest way to use the Vector API is through the Vector extension or CLI, which handle authentication and streaming for you.

### VS Code / JetBrains

Select **Vector** as your provider in the model picker dropdown. Sign in with your Vector account and your API key is managed automatically.
...
Because the Vector API is OpenAI-compatible, you can use it with any library or tool that supports custom OpenAI endpoints.
...
<CardGroup cols={2}>
  <Card title="CLI Reference" icon="terminal" href="/cline-cli/cli-reference">
    Full command reference for the Vector CLI, including auth setup.
  </Card>
