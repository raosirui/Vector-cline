...
description: "Use the Vector API from Python, Node.js, curl, the Vector CLI, and the VS Code extension."
---

The Vector API is OpenAI-compatible, so any library or tool that works with OpenAI also works with the Vector API. Just change the base URL and API key.
...
The [OpenAI Python SDK](https://github.com/openai/openai-python) works with the Vector API by setting `base_url`:
...
The [OpenAI Node.js SDK](https://github.com/openai/openai-node) works with the Vector API by setting `baseURL`:
...
The [Vector CLI](/cline-cli/cli-reference) is the fastest way to use the Vector API from your terminal. It handles authentication, streaming, and tool execution for you.
...
# Authenticate with a Vector API key
...
The Vector extension handles the API integration for you:

1. Open the Vector panel in your editor
2. Select **Vector** as the provider in the model picker
3. Sign in with your Vector account
4. Start chatting or give Vector a task

Your API key is managed automatically. No manual configuration needed.

For setup instructions, see [Installing Vector](/getting-started/installing-cline) and [Authorizing with Vector](/getting-started/authorizing-with-cline).
...
<CardGroup cols={2}>
  ...
  <Card title="CLI Reference" icon="terminal" href="/cline-cli/cli-reference">
    Complete Vector CLI command reference.
  </Card>
</CardGroup>
