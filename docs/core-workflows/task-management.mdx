...
Every interaction with Vector happens within a task. Tasks are self-contained work sessions that capture your entire conversation, code changes, command executions, and decisions.

## What are Tasks?

A task begins when you submit a prompt to Vector. Your prompt defines the goal, and Vector works toward it through conversation, code changes, and tool use. The quality of your initial prompt directly affects how well Vector performs - clear, specific prompts lead to better results.
...
<Tip>
Want to get better results from Vector? Learn how to write effective prompts in our [Prompt Module](https://cline.bot/learn).
</Tip>

## Scoping Your Tasks

Each task carries its own context: the conversation history, decisions made, and understanding built up over the session. How you scope your tasks directly affects how well Vector can help you.
...
Every AI model has a context window - a limit on how much information it can process at once. Think of it as Vector's working memory for the current task.

As you work, the context window fills up with:
- Your prompts and Vector's responses
- File contents Vector reads or edits
- Command outputs and tool results
- System instructions that guide Vector's behavior (including [Vector Rules](/customization/cline-rules))

When the context window approaches its limit, Vector automatically compresses older parts of the conversation to make room. This means very long tasks may lose some earlier details, though Vector preserves the most important context.
...
If your starting context seems high even for simple prompts, add a [`.clineignore`](/customization/clineignore) file to exclude dependencies, build artifacts, and other files Vector doesn't need. This can dramatically reduce your baseline token usage.
...
Knowing when to start fresh versus continue can feel unclear at first. As you work with Vector more, you'll develop an intuition for it. Use this table as a starting point:

| Scenario | Action | Why |
|----------|--------|-----|
| Switching to a different feature | **New task** | Clean context, focused responses |
| Building on work Vector just completed | **Continue** | Shared understanding preserved |
| Vector keeps going off-track | **New task** | Fighting context wastes time |
...
| Refining Vector's last output | **Continue** | Momentum and decisions preserved |

To start a new task, click the **+** button in the Vector sidebar or use the `/newtask` command. Your file changes are preserved through [checkpoints](/core-workflows/checkpoints), and you can reference previous tasks from history anytime.

## Understanding Task Costs

Every cloud-based AI model charges for usage based on tokens, the units of text the model processes. Vector tracks these costs automatically and displays them in the task header so you can monitor spending as you work.

### How Costs Are Calculated

When you interact with Vector, the model processes:
...
Cloud providers charge per million tokens, with output tokens typically costing more than input. Some providers also support **prompt caching**, which reduces costs when the same context (like your cline rules or large files) appears in multiple requests. Vector automatically tracks cache savings when available.
...
|--------------|---------------|
| **Vector Provider** | Pay-per-use with credits you purchase |
...
Not ready to pay? Vector offers several free paths:

- **Free models**: Search "free" in the model selector when using the Vector provider. These models display a **FREE** tag and work well for learning and experimentation.
...
Click the **History** button in the Vector sidebar (clock icon at the top-right) to open the history view. You'll see all your past tasks with their initial prompt, timestamp, and token usage. Each task card expands to show a preview of the conversation.

### Searching Tasks

Use the search bar at the top of the history view to find specific tasks. The fuzzy search looks across everything: your prompts, Vector's responses, code snippets, and file names. 
...
Vector can resume interrupted tasks with full context:

1. Open the task from history
2. Vector loads the complete conversation
...
This works across sessions. Even if you close the editor and return days later, Vector can pick up where you left off.
