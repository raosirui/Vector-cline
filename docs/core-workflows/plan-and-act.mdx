...
description: "Think first, then build. Vector's dual-mode system for structured development."
---
...
Plan mode is where you and Vector figure out what you're building and how. In this mode, Vector can read your codebase, run searches, and discuss strategy, but cannot modify any files or execute commands.
...
Once you have a plan, switch to Act mode. Vector retains the full context from your planning session and can now modify files, run commands, and execute your strategy.

The conversation history carries over when you switch modes. Vector remembers everything you discussed in Plan mode, so you don't need to repeat yourself. This makes the transition seamless.

<Note>
	While you can start directly in Act mode, planning first is highly recommended. The planning phase intentionally builds context that Vector needs to implement changes effectively. Without it, Vector may lack the understanding required to make the right decisions.
</Note>

## Typical Workflow

1. Start in Plan mode and describe what you want to build
2. Let Vector explore relevant files and understand the codebase
3. Discuss the approach, considering edge cases and potential issues
4. When confident in the plan, switch to Act mode
5. Vector implements the solution based on your planning session
...
For complex tasks that need thorough analysis, use the `/deep-planning` slash command. This triggers an extended planning session where Vector:
...
For complex features that span multiple files, require architectural decisions, or will take multiple sessions to complete, use the `/deep-planning` slash command. This creates a detailed implementation plan that Vector can reference throughout the work.
...
- Have Vector write a markdown file summarizing the plan for future reference
- Use [file mentions](/core-workflows/working-with-files) to point Vector at relevant files during planning
...
- For large tasks, ask Vector to create a todo list during planning that you can track in Act mode
