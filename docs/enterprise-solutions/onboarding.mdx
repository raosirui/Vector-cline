...
description: "This guide explains how administrators configure SSO provisioning and user management in Vector Enterprise."
---

## Overview
Vector Enterprise integrates with your existing identity provider (IdP) via WorkOS to deliver secure SSO and zero-touch user lifecycle management. In this guide, you'll connect your IdP (Okta, Azure AD, Google Workspace, or any SAML/OIDC provider), enable just-in-time (JIT) provisioning so new users are created automatically on first sign-in, and configure role mapping so permissions stay aligned with your directory-no manual invites or seat reconciliations required.


## Prerequisites

- [Vector Enterprise License](https://cline.bot/enterprise)
...
### Step 1: Onboard to Vector Enterprise license
...
<Info>
For a short overview of where SSO configuration lives (Vector dashboard vs WorkOS vs your IdP), see [SSO Setup](/enterprise-solutions/sso-setup).
</Info>
...
Vector Enterprise uses **just-in-time provisioning** that works automatically:
...
- **Admin** in IdP → **Admin** role in Vector (Note: The first Owner of the org is created manually during onboarding)
- **Member** in IdP → **Member** role in Vector

<Info>
    For what each role can access, see the [Roles and Permissions](/enterprise-solutions/team-management/managing-members) page.
</Info>

If needed, you can configure additional user attributes in the Vector Admin console:
...
1. In the WorkOS dashboard (or Vector Admin console if available), locate and click **Test SSO Connection**
...
Once SSO is configured, users in your IdP can access Vector automatically without manual invites or account setup.

**First-time sign-in flow:**

1. User navigates to Vector and clicks **Sign in with SSO**
2. User authenticates via your organization's IdP
3. Vector automatically creates their account in your Organization
4. Role is assigned based on their IdP role (see [Step 4](#step-4-configure-user-attributes-mapping))
5. User is redirected to Vector and can begin working
...
5. **Test User Deprovisioning**: Remove a user from your IdP and verify they lose access to Vector on their next login attempt
