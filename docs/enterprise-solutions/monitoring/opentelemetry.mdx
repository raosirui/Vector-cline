...
description: "Export Vector telemetry to your observability platform using OpenTelemetry Protocol (OTLP)"
---

Vector includes opt-in OpenTelemetry support for exporting metrics and logs to your own observability infrastructure using the OpenTelemetry Protocol (OTLP).
...
Vector's OpenTelemetry support allows you to:
...
Vector supports OpenTelemetry's **OTLP (OpenTelemetry Protocol)** export with:

<CardGroup cols={2}>
  <Card title="Metrics Export" icon="chart-bar">
    Export metrics about Vector usage, performance, and errors
  </Card>
...
Vector supports three OTLP export protocols:
...
Once you’ve done so, if you perform some actions that trigger metrics and/or logs (such as doing a task with Vector), 
...
When OpenTelemetry is enabled, Vector exports:
...
Current OpenTelemetry support in Vector:
