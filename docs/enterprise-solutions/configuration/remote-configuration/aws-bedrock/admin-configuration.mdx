...
description: "This guide explains how administrators configure AWS Bedrock as the organization-wide LLM provider for Vector."
---


As an administrator, you can add AWS Bedrock as the organization-wide LLM provider for all Vector users through the hosted admin console. This centralized approach ensures consistent access to Amazon's AI models while maintaining your organization's security and compliance requirements through region controls and basic configuration options.
...
Your AWS account needs specific Bedrock permissions to work with Vector. 
...
Once saved, all organization members signed into the Vector extension will automatically use AWS Bedrock with your configured settings. They won't be able to select other providers or switch to their personal Vector accounts.
