...
YOLO Mode enables Vector to operate with complete autonomy, auto-approving all actions without user confirmation. For Enterprise administrators, this page covers how to control access to YOLO Mode across your organization.
...
When YOLO Mode is enabled, Vector automatically approves all operations including file changes, terminal commands, browser actions, and mode transitions. This provides maximum automation speed but removes all safety guardrails.
...
- Users can enable or disable YOLO Mode in their local Vector settings
...
- Persists across Vector restarts
