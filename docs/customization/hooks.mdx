...
title: "Hooks"
sidebarTitle: "Hooks"
description: "Inject custom logic into Vector's workflow to validate operations and shape Vector's decisions."
---

Hooks are scripts that run at key moments in Vector's workflow. Because they execute at known points with consistent inputs and outputs, hooks bring determinism to the non-deterministic nature of AI models by enforcing guardrails, validations, and context injection. You can validate operations before they execute, monitor tool usage, and shape how Vector makes decisions.
...
- Add context to the conversation based on what Vector is doing

## Hook Types

Vector supports 8 hook types that run at different points in the task lifecycle:
...
| PreToolUse | Before Vector executes a tool (read_file, write_to_file, etc.) |
| PostToolUse | After a tool execution completes |
| UserPromptSubmit | When you submit a message to Vector |
| PreCompact | Before Vector truncates conversation history to free up context |
...
flowchart TD
    ...
    subgraph Conversation Cycle
        ...
        H_Submit --> Thinking[Vector Processes Context]:::state
...
2. **Conversation Cycle**: Each time you send a message, **UserPromptSubmit** runs, then Vector processes your request
3. **Tool Execution**: When Vector decides to use a tool, **PreToolUse** runs first-if allowed, the tool executes, then **PostToolUse** runs
...
- **Global hooks**: `~/Documents/Vector/Hooks/`
...
<Steps>
  <Step title="Open the Hooks tab">
    Click the scale icon at the bottom of the Vector panel, to the left of the model selector. Switch to the Hooks tab.
  </Step>
  ...
  <Step title="Review the hook's code">
    Click the pencil icon to open and edit the hook script. Vector generates a template with examples.
  </Step>
...
Let's create a simple hook that logs every file Vector reads or writes. You'll see results in seconds.
...
<Steps>
  <Step title="Create the hook file">
    Save the script above as `~/Documents/Vector/Hooks/file-logger` or create it through the Hooks UI.
  </Step>
  <Step title="Make it executable">
    On macOS/Linux, run `chmod +x ~/Documents/Vector/Hooks/file-logger`.
  </Step>
  <Step title="Enable it (macOS/Linux only)">
    In Vector's Hooks tab, find "file-logger" under PreToolUse hooks and toggle it on.
  </Step>
</Steps>
...
Ask Vector to read any file in your project: "What's in package.json?"
...
`model.provider` and `model.slug` are machine-stable identifiers for the active provider/model at hook execution time. If unavailable, Vector sends deterministic fallback values: `"unknown"`.
...
| `contextModification` | string | Optional text that gets injected into the conversation as context for Vector |
...
- Providing validation results that Vector should consider
...
Runs when you send a message to Vector. Use it to:
...
Runs before Vector truncates conversation history to stay within context limits. Use it to:
...
Hooks are available in the [Vector CLI](/cline-cli/getting-started):
