...
description: "Understand how Rules, Skills, Workflows, Hooks, and .clineignore work together to customize Vector."
---

Out of the box, Vector is a general-purpose AI assistant. Customizations transform it into an expert on your codebase, your team's conventions, and your workflows. Instead of repeating the same instructions every task, you define them once and Vector follows them automatically.

Vector offers five systems for this: Rules, Skills, Workflows, Hooks, and .clineignore. Each serves a different purpose and activates at different times.

## Quick Comparison

| Feature | Purpose | When Active | Best For |
|---------|---------|-------------|----------|
| **[Rules](/customization/cline-rules)** | Define how Vector behaves | Always (or contextually) | Coding standards, project constraints, team conventions |
...
**[Rules](/customization/cline-rules)** are always-on guidance. Use them when you want Vector to consistently follow certain patterns: coding standards, naming conventions, architectural constraints, or project-specific context. Rules shape *how* Vector works across all tasks. For example, a rule might say "always use TypeScript" or "follow the repository pattern for data access."

**[Skills](/customization/skills)** are domain expertise that loads only when relevant. Use them when you have extensive knowledge that would waste context if always active. Vector sees skill descriptions at startup and activates the full instructions only when your request matches. A data analysis skill might include pandas patterns, visualization preferences, and output formats that Vector only loads when you're working with data files.

**[Workflows](/customization/workflows)** are explicit task scripts you invoke on demand. Use them when you have a repeatable multi-step process that should run the same way every time. Type `/release.md` and Vector executes your release sequence: bump version, run tests, update changelog, commit, tag, push. Workflows define *what* to do, step by step.

**[Hooks](/customization/hooks)** are programmatic guardrails that run automatically at key moments. Use them when you need to validate, enforce, or extend Vector's behavior with custom code. A hook might block `.js` file creation in a TypeScript project, run linters before saves, or notify external services after deployments.

**[.clineignore](/customization/clineignore)** controls which files and directories Vector can access. Use it to exclude dependencies, build artifacts, generated files, and large data files from Vector's context. This reduces token usage, lowers costs, and keeps Vector focused on the code that matters. It works like `.gitignore`: add patterns to a `.clineignore` file in your project root and matching files are automatically excluded.

### Example: A Release Process

Consider how all five work together for releasing a new version:

1. **Rules** ensure Vector follows your team's commit message format and versioning policy
2. **Skills** offer deep knowledge about your CI/CD system that Vector loads when deployment questions arise
...
5. **.clineignore** keeps build artifacts, `node_modules/`, and generated files out of Vector's context so it stays focused
...
|--------|-----------------|------------------|
| Rules | `~/Documents/Vector/Rules/` | `.clinerules/` |
| Skills | `~/.cline/skills/` | `.cline/skills/` |
| Workflows | `~/Documents/Vector/Workflows/` | `.clinerules/workflows/` |
| Hooks | `~/Documents/Vector/Hooks/` | `.clinerules/hooks/` |
...
Customizations are powerful. They shape how Vector writes code, execute commands automatically, and influence every interaction. Treat customization files with the same scrutiny you'd give any code running in your environment.
