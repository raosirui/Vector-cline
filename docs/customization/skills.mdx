...
title: "Skills"
sidebarTitle: "Skills"
description: "Modular instruction sets that extend Vector's capabilities for specific tasks."
---

Skills are modular instruction sets that extend Vector's capabilities for specific tasks. Each skill packages detailed guidance, workflows, and optional resources that Vector loads only when relevant to your request.

Install multiple skills and Vector only loads what it needs. A deployment skill stays dormant until you ask about deploying. Unlike [rules](/customization/cline-rules) (which are always active), skills load on-demand so they don't consume context when you're working on something unrelated.
...
When you send a message, Vector sees a list of available skills with their descriptions. If your request matches a skill's description, Vector activates it using the `use_skill` tool, which loads the full instructions from SKILL.md.
...
Detailed instructions for Vector to follow when this skill is activated.
...
- `description` tells Vector when to use this skill (max 1024 characters)

## Creating a Skill

<Steps>
  <Step title="Open the Skills menu">
    Click the scale icon at the bottom of the Vector panel, to the left of the model selector. Switch to the Skills tab.
  </Step>
  <Step title="Create a new skill">
    Click "New skill..." and enter a name for your skill (e.g., `aws-deploy`). Vector creates a skill directory with a template `SKILL.md` file.
  </Step>
...
You can also create skills manually by creating the directory structure in your file system. Place skill directories in `.cline/skills/` (workspace) or `~/.cline/skills/` (global) and Vector will detect them automatically.

Put the important information first in your SKILL.md. Vector reads the file sequentially, so front-load the common cases. Use clear section headers like "## Error Handling" or "## Configuration" so Vector can scan for relevant sections.
...
The description determines when Vector activates the skill. A vague description means the skill won't trigger when you expect it to.
...
Keep SKILL.md under 5k tokens. If your skill needs more content, split it into separate files in a `docs/` directory and reference them from the main instructions. Vector loads referenced files only when needed.
...
Skills can include additional files that Vector accesses only when needed.
...
A deployment skill might have `docs/aws.md`, `docs/gcp.md`, and `docs/azure.md`. Vector loads only the relevant platform guide based on your request.
...
A project setup skill could include `templates/dockerfile`, `templates/docker-compose.yml`, and `templates/.env.example` that Vector customizes for each new project.
...
Vector reads documentation files using `read_file` when the instructions reference them. Scripts can be executed directly, and only the script's output enters the context window.
...
Skills transform Vector from a general-purpose assistant into a specialist that knows your domain. Start with one skill for a task you repeat often, test it, and iterate on the description until it triggers reliably.
