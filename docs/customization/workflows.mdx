...
Workflows are Markdown files that define a series of steps to guide Vector through repetitive or complex tasks. Type `/` followed by the workflow's filename to invoke it (e.g., `/deploy.md`).

Deploying, setting up a new project, running through a release checklist: these tasks often require remembering a dozen steps, running commands in the right order, and updating files manually. Mess up one step and you're debugging for an hour. Workflows turn those multi-step processes into one command. Type `/release.md` and Vector handles the version bump, runs tests, updates the changelog, commits, tags, and pushes. You just review and approve.
...
- **High-level**: "Run the test suite and fix any failures" lets Vector decide how to accomplish the goal
...
<Steps>
  <Step title="Open the Workflows menu">
    Click the scale icon at the bottom of the Vector panel, to the left of the model selector. Switch to the Workflows tab.
  </Step>
...
<Tip>
**Create workflows from completed tasks.** After finishing something you'll need to repeat, tell Vector: "Create a workflow for the process I just completed." Vector analyzes the conversation, identifies the steps, and generates the workflow file. Your accumulated context becomes reusable automation.
</Tip>

### Invoking Workflows

Type `/` in the chat input to see available workflows. Vector shows autocomplete suggestions as you type, so `/rel` would match `release-prep.md`. Select a workflow and press Enter to start it.

Vector executes each step in sequence, pausing for your approval when needed. You can stop a workflow at any point by rejecting a step.
...
**Global workflows** go in your system's Vector Workflows directory. Use these for personal productivity workflows you use across all projects.

### Global Workflows Directory

| Operating System | Default Location |
|------------------|------------------|
| Windows | `Documents\Vector\Workflows` |
| macOS | `~/Documents/Vector/Workflows` |
| Linux/WSL | `~/Documents/Vector/Workflows` |
...
Write steps as plain instructions. Vector interprets them and figures out which tools to use:
...
This approach works well when you want Vector to adapt to the situation rather than follow rigid steps.

### Vector Tools

For precise control, use Vector's built-in tools with XML syntax. This guarantees specific actions:
...
See the full list in the [Vector Tools Reference](/tools-reference/all-cline-tools).
...
Or describe the intent in natural language and let Vector figure out the tool call:
...
**Include failure handling.** Tell Vector what to do when something goes wrong: "If tests fail, show the failures and stop the workflow."
...
The workflow demonstrates both approaches: XML tool syntax (`<execute_command>`, `<ask_followup_question>`) for steps that need precise control, and natural language for steps where Vector should adapt to the situation.
...
Invoke it with `/release-prep.md` and Vector walks through each step.
