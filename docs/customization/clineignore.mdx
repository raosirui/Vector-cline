...
description: "Control which files and directories Vector can access in your project."
---

The `.clineignore` file tells Vector which files and directories to skip when analyzing your codebase. It works like `.gitignore`: create a file named `.clineignore` in your project root, add patterns for files you want excluded, and Vector will ignore them.

## Why It Matters

Without a `.clineignore`, Vector may load your entire project into context, including dependencies, build artifacts, and generated files. This wastes tokens, increases costs, and can push useful context out of the window.
...
- Configuration files Vector needs to understand (`tsconfig.json`, `package.json`)
- Documentation and READMEs
- Test files (Vector often needs these for context)

## How It Works

When Vector scans your project to build context, it checks each file path against your `.clineignore` patterns. Matching files are excluded from:

- The file listing Vector sees when starting a task
- Automatic context gathering during conversations
- Search results when Vector looks for relevant code

You can still reference ignored files explicitly using [@ mentions](/core-workflows/working-with-files). If you type `@/node_modules/some-package/index.js`, Vector will read that specific file even though `node_modules/` is in your `.clineignore`. The ignore rules control automatic loading, not explicit access.

<Note>
`.clineignore` is separate from `.gitignore`. Files tracked by Git but irrelevant to Vector (like large test fixtures or data files) should go in `.clineignore` even if they're not in `.gitignore`.
</Note>
...
- If Vector seems to be missing context about a file, check whether it's being excluded by your ignore patterns.
...
- [Vector Rules](/customization/cline-rules) - Define persistent instructions for Vector
