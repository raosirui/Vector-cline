...
Background Edit lets Vector make file changes without opening the diff editor, so you can keep writing code while Vector works on other files in the background.
...
By default, Vector opens a side-by-side diff editor tab for each file it modifies. With Background Edit enabled:
...
1. Click the settings icon (gear) in the top-right corner of the Vector panel
...
Stick with the default diff editor if you prefer reviewing each change before it saves, or need to make inline edits to Vector's proposed changes.
