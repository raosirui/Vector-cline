...
When your conversation approaches the model's context window limit, Vector automatically summarizes it to free up space and keep working.
...
Vector monitors token usage during your conversation. When you're getting close to the limit, it:
...
Previously, Vector would truncate older messages when hitting context limits, losing important context.
...
- Vector remembers everything it's done
- You can work on much larger projects without interruption

<Tip>
Auto Compact works beautifully with [Focus Chain](/features/focus-chain). When Focus Chain is enabled, todo lists persist across summarizations. Vector can work on long-horizon tasks spanning multiple context windows while staying on track.
</Tip>
...
<Note>
With other models, Vector falls back to standard rule-based context truncation, even if Auto Compact is enabled.
</Note>
