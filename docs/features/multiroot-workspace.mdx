...
Vector works with VSCode's multi-root workspaces, letting you manage multiple project folders or repositories in a single window. Whether you're working with a monorepo or separate Git repositories, Vector can read files, write code, and run commands across all of them.
...
- **Vector rules** only work in the primary workspace folder
...
Vector can complete tasks that span multiple projects or repositories:
...
Vector supports both patterns, as well as hybrid setups where some folders are Git repositories and others are not. The key difference: with multiple repositories, each folder has its own `.git` directory and Vector tracks them independently.
...
When you open separate Git repositories in one workspace, Vector treats each as an independent project with its own version control.

### What Vector Tracks Per Repository

For each workspace folder, Vector detects:
...
This means Vector understands that your frontend and backend might be at different commits, on different branches, or even use different version control systems.

<Note>
While Vector detects VCS information for all workspace folders, certain features only use the **primary workspace** (the first folder): [Vector rules](/customization/cline-rules), [workflows](/customization/workflows), and [Git-related features](/core-workflows/working-with-files) like `@git` mentions.
</Note>
...
Vector understands natural references to your workspaces:
...
- Vector needs to resolve ambiguity
...
Each service has its own repository. Vector can update the proto definitions and regenerate clients across all services.
...
The types repository defines interfaces used by both client and server. When you update a type, Vector can fix both consumers.
...
Mix of repositories and plain folders. Vector adapts to each folder's configuration.
...
[Vector rules](/customization/cline-rules) (`.clinerules/` directory) only work in the **primary workspace** (the first folder in your workspace). Rules in other workspace folders are ignored.

**Workaround:** Place shared rules in the primary workspace, or use global rules (`~/Documents/Vector/Rules/`) which apply everywhere.

### Checkpoints

[Checkpoints](/core-workflows/checkpoints) are disabled in multi-root workspace mode. Vector displays a warning when this happens.
...
4. **Consider the primary workspace** for Vector rules placement
...
- Use [Plan mode](/core-workflows/plan-and-act) to let Vector understand structure first
- Add a `.clineignore` file to reduce noise, speed up scanning, and keep Vector focused on source code:
