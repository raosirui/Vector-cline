...
Subagents let Vector spawn focused research agents that run in parallel. Each subagent gets its own prompt and context window, explores the codebase independently, and returns a detailed report to the main agent. This keeps the main agent's context clean while gathering broad information fast.
...
When Vector uses the `use_subagents` tool, it launches independent agents simultaneously. Each one:
...
1. Open Vector Settings (click the gear icon in the Vector panel)
2. Go to **Features**
...
Vector does not automatically decide to use subagents. You need to ask for them in your prompt. When the feature is enabled and you mention subagents (or describe a task that benefits from parallel exploration), Vector will use the `use_subagents` tool.
...
Each subagent prompt should describe a focused research question. Vector will run them in parallel and synthesize the results.
...
If auto-approve is off, Vector will ask for your approval before launching subagents, showing you the prompts it plans to send.
...
For small, focused tasks where you already know which files to look at, subagents add unnecessary overhead. Just ask Vector directly.
