...
Worktrees let you work on multiple branches simultaneously, each in its own folder. This enables Vector to work on tasks in parallel across separate VS Code windows, or lets Vector work independently while you continue coding in your main workspace.
...
## Why Use Worktrees with Vector?

Worktrees solve a common problem: **Vector takes over your VS Code window while working on a task**. With worktrees, you can:

1. **Run Vector in parallel** - Have Vector work on multiple tasks simultaneously, each in its own worktree and VS Code window
2. **Keep working while Vector works** - Let Vector handle a task in a separate worktree while you continue coding in your main workspace
...
The fastest way to start using worktrees is the **New Worktree Window** button on Vector's home screen:

1. Click **New Worktree Window** on the home screen
2. Enter a branch name and folder path (defaults are auto-filled)
3. Click **Create & Open**

A new VS Code window opens with your worktree, and Vector automatically opens ready to work.
...
For more control, open the full Worktrees view by clicking the **Worktrees** button in the Vector sidebar header, or by clicking your current branch info on the home screen:
...
<Steps>
  ...
  <Step title="Open in New Window">
    Once created, click the **Open in new window** button to open the worktree in a separate VS Code window. Vector will automatically open in the new window.
  </Step>
</Steps>
...
<Steps>
  <Step title="Create a new worktree">
    Click **New Worktree Window** on the home screen or use the Worktrees view. A new VS Code window opens with Vector ready to go.
  </Step>
  <Step title="Do your work">
    Work on your feature or let Vector handle a task. Make commits as you go.
  </Step>
...
- **Open in new window**: Open the worktree in a separate VS Code window (recommended for parallel Vector sessions)

Either way, Vector automatically opens in the new workspace, ready to start a task.
...
If your branch has conflicts with the main branch, Vector will detect them and show you the conflicting files. You have two options:

1. **Ask Vector to Resolve & Merge** - Creates a new Vector task with a prompt asking Vector to resolve the conflicts, complete the merge, and clean up the worktree
2. **Resolve Manually** - Close the modal and resolve conflicts yourself using your preferred Git tools

<Tip>
  The "Ask Vector to Resolve" option is particularly useful for complex conflicts. Vector will analyze the conflicting files and attempt to merge them intelligently based on the intent of both branches.
</Tip>
...
3. When Vector creates a new worktree, files matching **both** `.worktreeinclude` **and** `.gitignore` are copied automatically
...
<AccordionGroup>
  <Accordion title="For Parallel Vector Sessions">
...
## Using Worktrees with Vector CLI

Vector CLI's `--cwd` flag unlocks powerful command-line worktree workflows:

- **Parallel execution**: Run multiple Vector instances simultaneously in different worktrees
...
<AccordionGroup>
  ...
  <Accordion title="Relationship with Checkpoints">
    Worktrees are separate from Vector's [checkpoint system](/core-workflows/checkpoints). Each worktree has its own checkpoint history. Checkpoints track changes within a single worktree, while worktrees let you work across multiple branches simultaneously.
  </Accordion>
</AccordionGroup>

Worktrees unlock true parallel development with Vector. Create a worktree, open it in a new window, and let Vector work independently while you continue coding!
