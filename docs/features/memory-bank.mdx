...
description: "A structured documentation system that helps Vector maintain context across sessions."
---

Memory Bank is a documentation methodology that transforms Vector from a stateless assistant into a persistent development partner. Through structured markdown files, Vector can "remember" your project details across sessions.
...
3. Ask Vector to "initialize memory bank"

## How It Works

Memory Bank files are regular markdown files in your project that both you and Vector can access. They're organized hierarchically to build a complete picture of your project:
...
- **"follow your custom instructions"** - Tells Vector to read Memory Bank and continue where you left off
...
These work alongside Vector's built-in [slash commands](/core-workflows/using-commands). In particular, [`/newtask`](/core-workflows/using-commands#newtask) and [`/smol`](/core-workflows/using-commands#smol) help you manage context windows without losing progress.

## Working with Plan & Act Modes

Memory Bank pairs naturally with [Plan & Act mode](/core-workflows/plan-and-act):

- **Plan mode**: Start here when resuming a project. Ask Vector to read the Memory Bank, review the current state, and discuss strategy before making changes.
- **Act mode**: Switch to Act mode once you have a plan. Vector retains everything from the planning session and can implement changes.

For complex features, use [`/deep-planning`](/core-workflows/using-commands#deep-planning) to have Vector investigate your codebase and create a detailed implementation plan. The Memory Bank gives Vector the project context it needs to plan effectively.
...
1. Ask Vector to "update memory bank" to document the current state
2. Start a new conversation
3. Ask Vector to "follow your custom instructions"
...
Vector's built-in commands offer more targeted options:
...
Enable [Auto-Compact](/features/auto-compact) to let Vector automatically compress context as you work. This reduces how often you need to manually manage the context window, though you should still update the Memory Bank after significant milestones.
...
Memory Bank works best when your starting context is lean. If Vector loads your entire project into context, including dependencies, build artifacts, and generated files, you burn through tokens before the real work starts.

**Add a [`.clineignore`](/customization/clineignore) file.** This is the single biggest improvement most users can make. It tells Vector which files to skip when scanning your project. Adding one can drop your starting context from 200k+ tokens to under 50k, which means faster responses, lower costs, and the ability to use smaller models effectively.

**Keep Memory Bank files concise.** Each file adds to your context when Vector reads it at the start of a session. Keep `projectbrief.md` to one page, `activeContext.md` to current state only (not a running log), and `progress.md` to a summary rather than a detailed changelog. If a file grows beyond a page or two, split the detail into a separate doc and link to it. Vector can read linked files on demand.

**Use [Vector Rules](/customization/cline-rules) strategically.** Rules load into every request. Use [conditional rules](/customization/cline-rules#conditional-rules) to activate rules only when working with matching files, so frontend rules don't load when you're editing backend code.

## Best Practices

- Start with a basic project brief and let structure evolve
- Let Vector help create the initial structure
...
- Use [Vector Rules](/customization/cline-rules) to store the Memory Bank instructions per-project
...
```markdown
# Vector's Memory Bank

I am Vector, an expert software engineer with a unique characteristic: my memory resets completely between sessions. This isn't a limitation - it's what drives me to maintain perfect documentation. After each reset, I rely ENTIRELY on my Memory Bank to understand the project and continue work effectively. I MUST read ALL memory bank files at the start of EVERY task - this is not optional.
...
Memory Bank stores important information in structured files that Vector can load efficiently at the start of each session. This prevents context bloat while keeping critical information available. For more on how context windows work, see [Task Management](/core-workflows/task-management#context-window).
...
- [Vector Rules](/customization/cline-rules) - Define persistent instructions including Memory Bank setup
