...
description: "Transform Vector into a meticulous architect who investigates your codebase and creates comprehensive implementation plans."
---

Deep Planning (`/deep-planning`) turns Vector into an architect before it becomes a builder. Instead of jumping straight into code, Vector systematically explores your codebase, asks targeted questions, and produces a detailed implementation plan — all before writing a single line.
...
<Steps>
  <Step title="Silent Investigation">
    Vector explores your codebase without asking you anything. It reads relevant files, traces dependencies, examines patterns, and builds a mental model of how your project is structured. You'll see Vector reading files and running searches during this phase.

    This step is intentionally silent — Vector gathers context first so it can ask better questions next.
  </Step>

  <Step title="Discussion">
    Based on what it learned, Vector asks you targeted, specific questions about your requirements and preferences. These aren't generic questions — they're informed by what Vector found in your code.

    For example, instead of asking "how should authentication work?", Vector might ask "I see you're using JWT tokens in `auth/middleware.ts` with refresh token rotation. Should the new endpoint follow the same pattern, or do you want session-based auth for this feature?"
  ...
  <Step title="Plan Creation">
    Vector generates a comprehensive `implementation_plan.md` file in your project. This plan typically includes:
  ...
  <Step title="Task Creation">
    After you approve the plan, Vector creates a new task with the implementation steps loaded as trackable items. This gives you a clean context window focused entirely on execution, with the plan serving as the roadmap.
  </Step>
</Steps>
...
Type `/deep-planning` in the Vector chat input, followed by a description of what you want to build:
...
Once Vector generates `implementation_plan.md`, review it carefully:
...
Tell Vector about any adjustments before proceeding to implementation.

## Model-Specific Optimization

The deep planning prompt is optimized for each model family. Vector adapts its investigation and planning approach based on the strengths of whatever model you're using — whether that's Claude, GPT, Gemini, DeepSeek, or others.
...
<Tip>
Consider using a stronger reasoning model for the planning phase and a faster model for implementation. You can configure separate models for Plan and Act modes in Vector Settings. See [Plan & Act Mode](/core-workflows/plan-and-act#using-different-models-for-each-mode) for details.
</Tip>

## Pairing with Other Features

Deep Planning works well with several other Vector features:
...
<Tip>
A powerful workflow: run `/deep-planning` to create the plan, enable [Focus Chain](/features/focus-chain) to track progress, then let Vector implement step by step. You get architecture-level thinking with granular progress visibility.
</Tip>
...
- **Point Vector at relevant files.** Use `@` mentions to highlight key files in your prompt so the investigation phase starts in the right place.
- **Edit the plan before implementing.** The generated plan is a starting point. Adjust priorities, remove unnecessary steps, or add details before Vector starts coding.
...
- **Use for onboarding.** Run `/deep-planning` on a feature you're unfamiliar with to get Vector to map out the codebase and explain how things connect.

## Related

- [Plan & Act Mode](/core-workflows/plan-and-act) — Vector's dual-mode system for structured development
