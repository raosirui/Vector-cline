...
description: "Control which actions Vector can take without asking for approval."
---

Auto Approve lets you decide which actions Vector can take without prompting you each time. It keeps you out of approval popups during routine work while letting you keep tight control over high-risk actions.
...
Auto Approve is evaluated per tool call. When Vector is about to read a file, edit a file, run a command, or use the browser, it checks your Auto Approve settings for that category.
...
- **Terminal commands**: Vector treats commands as either safe or requiring approval. "Execute safe commands" covers safe commands. "Execute all commands" extends to commands flagged as requiring approval.
- **Notifications**: If enabled, Vector sends OS-level notifications when approval is required, and when an auto-approved terminal command has been running for 30 seconds.
...
Vector does not use a fixed allowlist. The model marks each command with a `requires_approval` flag based on the command and arguments. These are examples, not guarantees.
...
YOLO mode is Auto Approve on steroids. Check the box and Vector auto-approves everything: file changes, terminal commands, browser actions, MCP tools, and mode transitions.

<Note>
**Warning: This is dangerous.** YOLO mode disables all safety checks. Vector executes whatever it decides without asking permission.
</Note>
...
**Trusted, repetitive tasks** where you've validated Vector's approach and want to eliminate approval overhead.

**Demonstration purposes** where you want to show Vector's capabilities without interruptions.
...
- **Monitor the output.** Vector still shows what it's doing. Watch the terminal and file changes.
- **Keep version control handy.** Git becomes your safety net.

### Enabling YOLO Mode

Navigate to Vector Settings, then Features, and check "YOLO Mode." No confirmation dialogs. Once enabled, Vector auto-approves all actions immediately. Uncheck to disable.
