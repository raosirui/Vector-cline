 ...
 */
export async function initialize(storageContext) {
    // Configure the shared Logging class to use HostProvider's output channels and debug logger
    Logger.subscribe((msg) => HostProvider.get().logToChannel(msg)); // File system logging
    Logger.subscribe((msg) => HostProvider.env.debugLog({ value: msg })); // Host debug logging
...
async function showVersionUpdateAnnouncement(stateManager) {
 ...
 */
async function checkWorktreeAutoOpen(stateManager) {
    try {
