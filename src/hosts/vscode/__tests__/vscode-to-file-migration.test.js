...
describe("vscode-to-file-migration", () => {
	...
	describe("sentinel behavior", () => {
		it("should migrate on first run (no sentinel)", async () => {
			...
			const result = await exportVSCodeStorageToSharedFiles(mockCtx, storageContext)
		...
		it("should migrate workspace state when globals already migrated (new workspace)", async () => {
			...
			const stored = storageContext.workspaceState.get("localClineRulesToggles")
	...
	describe("error handling", () => {
		it("should NOT write sentinel if migration throws", async () => {
			...
			try {
				await exportVSCodeStorageToSharedFiles(mockCtx, storageContext)
	...
	describe("idempotency", () => {
		it("should produce same result when run twice", async () => {
			...
			// First run
			const result1 = await exportVSCodeStorageToSharedFiles(mockCtx, storageContext)
			result1.migrated.should.be.true()
			// Second run — should be skipped due to sentinel
			const result2 = await exportVSCodeStorageToSharedFiles(mockCtx, storageContext)
...
describe("createStorageContext", () => {
	...
	it("should store and retrieve values correctly", () => {
		...
		const ws = ctx.workspaceState.get("wsKey")
		ws.toggle.should.equal(true)
	})
})
//# sourceMappingURL=vscode-to-file-migration.test.js.map
