...
import { DEFAULT_RECONNECT_CONFIG, StreamableHttpReconnectHandler, } from "../StreamableHttpReconnectHandler";
...
function makeCallbacks(connection) {
...
describe("StreamableHttpReconnectHandler", () => {
    ...
    it("should exhaust retries when called with attempts already at max", async () => {
        const conn = makeConnection();
        const cbs = makeCallbacks(conn);
        const config = { maxAttempts: 0, getDelayMs: () => 0 };
...
//# sourceMappingURL=StreamableHttpReconnectHandler.test.js.map