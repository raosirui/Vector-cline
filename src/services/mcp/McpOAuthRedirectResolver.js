...
export class McpOAuthRedirectResolver {
     ...
     */
    static extractLoopbackPort(url) {
     ...
     */
    static isRedirectCompatible(savedRedirectUrl, currentRedirectUrl) {
     ...
     */
    static async resolve(savedRedirectUrl, callbackPath, getCallbackUrl) {
        // Determine if we have a preferred port to try
        const preferredPort = savedRedirectUrl !== undefined ? McpOAuthRedirectResolver.extractLoopbackPort(savedRedirectUrl) : undefined;
        ...
        if (savedRedirectUrl !== undefined && !isRegistrationValid) {
            Logger.log(`[McpOAuthRedirectResolver] Redirect URL changed: saved="${savedRedirectUrl}" current="${redirectUrl}" — client re-registration required`);
        }
        return { redirectUrl, isRegistrationValid };
    }
}
//# sourceMappingURL=McpOAuthRedirectResolver.js.map