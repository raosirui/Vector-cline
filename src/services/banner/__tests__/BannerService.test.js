...
describe("BannerService", () => {
    ...
    beforeEach(() => {
        ...
        sandbox.stub(StateManager, "get").returns({
            ...
            getGlobalSettingsKey: (key) => (key === "mode" ? mockStateManagerConfig.mode : undefined),
            getGlobalStateKey: (key) => (key === "dismissedBanners" ? mockStateManagerConfig.dismissedBanners : []),
            setGlobalState: (key, value) => {
                if (key === "dismissedBanners") {
                    mockStateManagerConfig.dismissedBanners = value;
                }
            },
        });
    ...
    describe("API Fetching", () => {
        ...
        it("should cache banners for the configured hours from PostHog", async () => {
            ...
            flagPayloadStub.callsFake((flag) => flag === FeatureFlag.EXTENSION_REMOTE_BANNERS_TTL ? 4 * 60 * 60 * 1000 : undefined);
            const mockResponse = {
                data: {
                    items: [
                        {
                    ...
                    items: [
                        {
                            ...
                            severity: "info",
                            placement: "top",
                            rulesJson: JSON.stringify({ providers: ["anthropic"] }),
                        },
                    ],
                },
            };
    ...
    describe("IDE Type Detection", () => {
        function stubHostInfo(overrides) {
            ;
            HostRegistryInfo.get.returns({
                extensionVersion: "1.0.0",
        ...
        async function getIdeParam(fetch) {
