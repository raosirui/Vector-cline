 ...
 */
async function getCurrentWorktreePath(cwd) {
    try {
        const git = simpleGit(cwd);
 ...
 */
export async function getGitRootPath(cwd) {
 ...
 */
export async function createWorktree(cwd, worktreePath, options = {}) {
    const isInstalled = await checkGitInstalled();
    if (!isInstalled) {
 ...
 * Delete a worktree
 */
export async function deleteWorktree(cwd, path, force = false) {
    const isInstalled = await checkGitInstalled();
    if (!isInstalled) {
        return { success: false, message: "Git is not installed" };
    }
 ...
 */
export async function getAvailableBranches(cwd) {
    const isInstalled = await checkGitInstalled();
    if (!isInstalled) {
