...
export class TaskCheckpointManager {
    ...
    async saveCheckpoint(isAttemptCompletionMessage = false, completionMessageTs) {
        try {
            ...
            if (!this.state.checkpointTracker) {
                Logger.error(`[TaskCheckpointManager] Failed to save checkpoint for task ${this.task.taskId}: Checkpoint tracker not available`);
                return;
            }
            ...
            if (!isAttemptCompletionMessage) {
                ...
                if (messageTs) {
                    ...
                    if (targetMessage) {
                        this.state.checkpointTracker
                            ...
                            .catch((error) => {
                            Logger.error(`[TaskCheckpointManager] Failed to create checkpoint commit for task ${this.task.taskId}:`, error);
                        });
                    }
                }
            }
                ...
                if (this.state.checkpointTracker) {
                    ...
                    Logger.error(`[TaskCheckpointManager] Checkpoint tracker does not exist and could not be initialized for attempt completion for task ${this.task.taskId}`);
                }
            }
        }
        catch (error) {
                ...
                case "workspace":
                    ...
                    if (!this.state.checkpointTracker && !this.state.checkpointManagerErrorMessage) {
                        try {
                            ...
                            const errorMessage = error instanceof Error ? error.message : "Unknown error";
                            Logger.error(`[TaskCheckpointManager] Failed to initialize checkpoint tracker for task ${this.task.taskId}:`, errorMessage);
                    ...
                    if (message.lastCheckpointHash && this.state.checkpointTracker) {
                        ...
                        try {
                            ...
                            const errorMessage = error instanceof Error ? error.message : "Unknown error";
                            Logger.error(`[TaskCheckpointManager] Failed to restore offset checkpoint for task ${this.task.taskId}:`, errorMessage);
                            HostProvider.window.showMessage({
                                type: ShowMessageType.ERROR,
                        ...
                        Logger.warn(`[TaskCheckpointManager] Message ${messageTs} has no checkpoint hash, falling back to previous checkpoint for task ${this.task.taskId}`);
                        try {
                            await this.state.checkpointTracker.resetHead(lastMessageWithHash.lastCheckpointHash);
                        }
                        catch (error) {
                            const errorMessage = error instanceof Error ? error.message : "Unknown error";
                            Logger.error(`[TaskCheckpointManager] Failed to restore fallback checkpoint for task ${this.task.taskId}:`, errorMessage);
                            HostProvider.window.showMessage({
                                type: ShowMessageType.ERROR,
                                message: "Failed to restore checkpoint: " + errorMessage,
                            });
    ...
    async presentMultifileDiff(messageTs, seeNewChangesSinceLastTaskCompletion) {
        ...
        try {
            ...
            if (!hash) {
                Logger.error(`[TaskCheckpointManager] No checkpoint hash found for message ${messageTs} in task ${this.task.taskId}`);
                relinquishButton();
                return;
            }
    ...
    async getWorkspacePath() {
        ...
        if (this.services.workspaceManager) {
            try {
                ...
                Logger.warn(`[TaskCheckpointManager] Failed to get workspace path from WorkspaceRootManager for task ${this.task.taskId}:`, error);
            }
        }
