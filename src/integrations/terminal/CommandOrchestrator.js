...
import { BUFFER_STUCK_TIMEOUT_MS, CHUNK_BYTE_SIZE, CHUNK_DEBOUNCE_MS, CHUNK_LINE_COUNT, COMPLETION_TIMEOUT_MS, MAX_BYTES_BEFORE_FILE, MAX_LINES_BEFORE_FILE, SUMMARY_LINES_TO_KEEP, } from "./constants";
/**
 ...
 */
export async function orchestrateCommandExecution(process, terminalManager, callbacks, options) {
    const { timeoutSeconds, onOutputLine, showShellIntegrationSuggestion, onProceedWhileRunning, terminalType = "vscode", suppressUserInteraction = false, } = options;
    const say = async (type, text, images, files, partial) => {
        if (suppressUserInteraction) {
            return undefined;
        }
        return callbacks.say(type, text, images, files, partial);
    };
    const ask = async (type, text, partial) => {
        if (suppressUserInteraction) {
            return undefined;
        }
    ...
    let backgroundTrackingResult = null; // Set when background tracking returns early
        ...
        if (!didContinue) {
            ...
            try {
                ...
                if (response === "yesButtonClicked") {
                    ...
                    telemetryService.captureTerminalUserIntervention(TerminalUserInterventionAction.PROCESS_WHILE_RUNNING, terminalType);
    ...
    let largeOutputLogPath = null;
    let largeOutputLogStream = null;
    let totalOutputBytes = 0;
    let totalLineCount = 0;
    let firstLines = []; // Keep first N lines for summary
    let lastLines = []; // Keep last N lines for summary (circular buffer)
    /**
    ...
    process.once("completed", async (details) => {
        completed = true;
    ...
    if (didCancelViaUi) {
        return {
            userRejected: true,
            result: formatResponse.toolResult(`Command cancelled. ${result.length > 0 ? `\nOutput captured before cancellation:\n${result}` : ""}`),
            completed: false,
    ...
    if (userFeedback) {
        ...
        return {
            userRejected: true,
            result: formatResponse.toolResult(`Command is still running in the user's terminal.${result.length > 0 ? `\nHere's the output so far:\n${result}` : ""}\n\nThe user provided the following feedback:\n<feedback>\n${userFeedback.text}\n</feedback>`, userFeedback.images, fileContentString),
            completed: false,
            outputLines: resultOutputLines,
            logFilePath: largeOutputLogPath || undefined,
            exitCode: completionDetails?.exitCode,
...
//# sourceMappingURL=CommandOrchestrator.js.map