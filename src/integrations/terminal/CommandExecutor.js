...
export class CommandExecutor {
    ...
    constructor(config, callbacks) {
        ...
        if (config.terminalExecutionMode === "backgroundExec" && config.terminalManager instanceof StandaloneTerminalManager) {
            ...
            if ("shellIntegrationTimeout" in config.terminalManager) {
                const tm = config.terminalManager;
     ...
     */
    async execute(command, timeoutSeconds, options) {
        ...
        if (this.wasCancelledExternally) {
            const outputSoFar = result.outputLines.length > 0
    ...
    async cancelBackgroundCommand() {
        ...
        if (this.currentProcess && typeof this.currentProcess.terminate === "function") {
    ...
    shouldShowBackgroundTerminalSuggestion() {
        ...
        this.shellIntegrationWarningTracker.timestamps = this.shellIntegrationWarningTracker.timestamps.filter((ts) => ts > oneHourAgo);
        // Add current warning
        this.shellIntegrationWarningTracker.timestamps.push(Date.now());
        // Check if we've shown suggestion recently (within last hour)
        if (this.shellIntegrationWarningTracker.lastSuggestionShown &&
