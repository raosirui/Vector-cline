...
import { cleanupMcpMarketplaceCatalogFromGlobalState, cleanupOldApiKey, migrateCustomInstructionsToGlobalRules, migrateTaskHistoryToFile, migrateWelcomeViewCompleted, migrateWorkspaceToGlobalStorage, } from "./core/storage/state-migrations";
...
export async function activate(context) {
    ...
    const webview = (await initialize(storageContext));
    ...
    */
    ...
    context.subscriptions.push(vscode.languages.registerCodeActionsProvider("*", new (class {
        ...
        provideCodeActions(document, range, context) {
            ...
            const addAction = new vscode.CodeAction(`Add to ${BRAND_NAME}`, vscode.CodeActionKind.QuickFix);
            addAction.command = {
                command: commands.AddToChat,
                title: `Add to ${BRAND_NAME}`,
                arguments: [expandedRange, context.diagnostics],
            };
            actions.push(addAction);
            // Explain with Cline (Always available)
            const explainAction = new vscode.CodeAction(`Explain with ${BRAND_NAME}`, vscode.CodeActionKind.RefactorExtract); // Using a refactor kind
            explainAction.command = {
                command: commands.ExplainCode,
                title: `Explain with ${BRAND_NAME}`,
                arguments: [expandedRange],
            };
            actions.push(explainAction);
            // Improve with Cline (Always available)
            const improveAction = new vscode.CodeAction(`Improve with ${BRAND_NAME}`, vscode.CodeActionKind.RefactorRewrite); // Using a refactor kind
            improveAction.command = {
                command: commands.ImproveCode,
                title: `Improve with ${BRAND_NAME}`,
                arguments: [expandedRange],
            };
            actions.push(improveAction);
            // Fix with Cline (Only if diagnostics exist)
            if (context.diagnostics.length > 0) {
                const fixAction = new vscode.CodeAction(`Fix with ${BRAND_NAME}`, vscode.CodeActionKind.QuickFix);
                fixAction.isPreferred = true;
                fixAction.command = {
                    command: commands.FixWithCline,
                    title: `Fix with ${BRAND_NAME}`,
                    arguments: [expandedRange, context.diagnostics],
                };
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.FocusChatInput, async (preserveEditorFocus = false) => {
...
* Always match the exact JSON format including quotes, commas, and escaped newlines.`;
    ...
    async function getNotebookCommandContext(range, diagnostics) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.JupyterGenerateCell, async (range, diagnostics) => {
        const userPrompt = await showJupyterPromptInput("Generate Notebook Cell", "Enter your prompt for generating notebook cell (press Enter to confirm & Esc to cancel)");
        if (!userPrompt)
            return;
...
\`\`\``;
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.JupyterExplainCell, async (range, diagnostics) => {
        const ctx = await getNotebookCommandContext(range, diagnostics);
        if (!ctx)
            return;
    ...
    context.subscriptions.push(vscode.commands.registerCommand(commands.JupyterImproveCell, async (range, diagnostics) => {
        const userPrompt = await showJupyterPromptInput("Improve Notebook Cell", "Enter your prompt for improving the current notebook cell (press Enter to confirm & Esc to cancel)");
        if (!userPrompt)
            return;
        const ctx = await getNotebookCommandContext(range, diagnostics);
        if (!ctx)
...
\`\`\``;
    ...
    const unsubSecrets = storageContext.secrets.onDidChange((event) => {
        if (event.key === "cline:clineAccountId") {
            const secretValue = storageContext.secrets.get(event.key);
...
async function showJupyterPromptInput(title, placeholder) {
...
function setupHostProvider(context) {
    ...
    outputChannel.appendLine(`[${BRAND_NAME}] Setting up VS Code host...`);
    ...
    const getCallbackUrl = async (path, _preferredPort) => {
    ...
    HostProvider.initialize(createWebview, createDiffView, createCommentReview, createTerminalManager, vscodeHostBridgeClient, () => { }, // No-op logger, logging is handled via HostProvider.env.debugLog
    getCallbackUrl, getBinaryLocation, context.extensionUri.fsPath, context.globalStorageUri.fsPath);
}
...
async function openClineSidebarForTaskUri() {
...
async function cleanupLegacyVSCodeStorage(context) {
    try {
