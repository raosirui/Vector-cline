...
const evaluatePathsConditional = (frontmatterValue, context) => {
...
export function evaluateRuleConditionals(frontmatter, context) {
 ...
 */
export function extractPathLikeStrings(text) {
    if (!text)
        return [];
    ...
    const tokenRegex = /(?:^|[\s([{"'`])((?:[A-Za-z0-9_.-]+(?:\/[A-Za-z0-9_.-]+)+\/?|[A-Za-z0-9_.-]+\.[A-Za-z0-9]{1,10}))(?=$|[\s)\]}"'`,.;:!?])/g;
    const matches = [];
    let match;
 ...
 */
export function toWorkspaceRelativePosixPath(absPath, workspaceRoot) {
...
//# sourceMappingURL=rule-conditionals.js.map