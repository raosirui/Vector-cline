...
export const getRuleFilesTotalContentWithMetadata = async (rulesFilePaths, basePath, toggles, opts) => {
    const evaluationContext = opts?.evaluationContext ?? {};
    const prefix = RULE_SOURCE_PREFIX[opts?.ruleNamePrefix ?? "global"];
    const parts = await Promise.all(rulesFilePaths.map(async (filePath) => {
        ...
        const activatedRule = hadFrontmatter && Object.keys(matchedConditions).length > 0
...
export function getRemoteRulesTotalContentWithMetadata(remoteRules, remoteToggles, opts) {
