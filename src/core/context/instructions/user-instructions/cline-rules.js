import { getRemoteRulesTotalContentWithMetadata, getRuleFilesTotalContentWithMetadata, RULE_SOURCE_PREFIX, synchronizeRuleToggles, } from "@core/context/instructions/user-instructions/rule-helpers";
...
import { evaluateRuleConditionals } from "./rule-conditionals";
export const getGlobalClineRules = async (globalClineRulesFilePath, toggles, opts) => {
    let combinedContent = "";
    ...
    if (await fileExistsAtPath(globalClineRulesFilePath)) {
        if (await isDirectory(globalClineRulesFilePath)) {
            try {
                ...
                const rulesFilesTotal = await getRuleFilesTotalContentWithMetadata(rulesFilePaths, globalClineRulesFilePath, toggles, {
...
export const getLocalClineRules = async (cwd, toggles, opts) => {
                ...
                if (clineRulesFilePath in toggles && toggles[clineRulesFilePath] !== false) {
                    ...
                    if (raw) {
                        ...
                        if (parsed.hadFrontmatter && parsed.parseError) {
                            ...
                            const { passed, matchedConditions } = evaluateRuleConditionals(parsed.data, opts?.evaluationContext ?? {});
                            if (passed) {
