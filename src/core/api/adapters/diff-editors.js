 ...
 */
export function convertApplyPatchToolCalls(messages) {
        ...
        const convertedContent = message.content.map((block) => {
            ...
            if (block.type === "tool_result") {
                ...
                if (conversion) {
                    ...
                    const reconstructedContent = reconstructApplyPatchResult(block, conversion.name, conversion.input, conversion.originalInput);
                    return {
                        ...block,
 ...
 */
function convertApplyPatchToToolCalls(input) {
 ...
 */
function extractNewContentFromPatch(patchContent) {
 ...
 */
function convertPatchToDiff(patchContent) {
    const diffBlocks = [];
    const lines = patchContent.split("\n");
    let i = 0;
 ...
 */
function reconstructApplyPatchResult(block, convertedToolName, _convertedInput, originalInput) {
 ...
 */
export function convertWriteToFileToolCalls(messages) {
    // Map to track tool_use_id to converted tool info and original input
    const toolUseIdMap = new Map();
    ...
    return messages.map((message) => {
        ...
        const convertedContent = message.content.map((block) => {
            ...
            if (block.type === "tool_result") {
                ...
                if (conversion) {
                    // Reconstruct the tool_result content to match apply_patch format
                    const reconstructedContent = reconstructWriteToFileResult(block, conversion.originalName, conversion.originalInput);
                    return {
                        ...block,
                        content: reconstructedContent,
                    };
 ...
 */
function convertToPatchFormat(toolName, input, finalContent) {
 ...
 */
function convertDiffToPatchWithContext(diff, finalContent) {
    const patchLines = [];
    ...
    while ((match = blockRegex.exec(diff)) !== null) {
        ...
        let prefixEnd = 0;
        while (prefixEnd < searchLines.length &&
 ...
 */
function reconstructWriteToFileResult(block, originalToolName, originalInput) {
    // Extract the content from the tool_result
    const content = typeof block.content === "string" ? block.content : "";
    // Try to extract the final_file_content
    const finalContentMatch = content.match(/<final_file_content path="([^"]+)">\s*([\s\S]*?)\s*<\/final_file_content>/);
...
//# sourceMappingURL=diff-editors.js.map