...
export class VercelAIGatewayHandler {
    ...
    async *createMessage(systemPrompt, messages, tools) {
        ...
        try {
            const stream = await createVercelAIGatewayStream(client, systemPrompt, messages, { id: modelId, info: modelInfo }, this.options.reasoningEffort, this.options.thinkingBudgetTokens, tools);
