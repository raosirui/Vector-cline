...
export class OpenAiNativeHandler {
    ...
    buildResponseCreateParams(args) {
        const requestedEffort = normalizeOpenaiReasoningEffort(this.options.reasoningEffort);
        const reasoning = requestedEffort === "none"
            ? undefined
            : {
    ...
    async *createResponseStreamHttp(modelInfo, params) {
        const client = this.ensureClient();
    ...
    async *createResponseStreamWebsocket(modelInfo, primaryParams, fallbackParams) {
    ...
    async ensureResponsesWebsocket() {
    ...
    async *createResponseEventsViaWebsocket(params) {
        ...
        const handleMessage = (evt) => {
            try {
                ...
                if (parsed?.type === "error" && parsed?.error) {
                    const error = new Error(parsed.error.message || "Responses websocket error");
        ...
        const handleError = () => {
            const error = new Error("Responses websocket emitted an error event");
        ...
        const handleClose = () => {
            if (!completed) {
                const error = new Error("Responses websocket closed during response stream");
    ...
    async *processResponsesEvents(stream, modelInfo) {
        ...
        for await (const chunk of stream) {
            ...
            if (chunk.type === "response.completed" && chunk.response?.usage) {
                ...
                const totalCost = calculateApiCostOpenAI(modelInfo, inputTokens, outputTokens + reasoningTokens, cacheWriteTokens, cacheReadTokens);
                const nonCachedInputTokens = Math.max(0, inputTokens - cacheReadTokens - cacheWriteTokens);
                yield {
                    type: "usage",
                    inputTokens: nonCachedInputTokens,
