...
import { resolveVectorProviderModelId } from "@/shared/vector-provider"
	...
	try {
		...
		refreshClineModels(controller).then(async (models) => {
			if (models && Object.keys(models).length > 0) {
				...
				const planActSeparateModelsSetting = controller.stateManager.getGlobalSettingsKey("planActSeparateModelsSetting")
				const currentMode = controller.stateManager.getGlobalSettingsKey("mode")
				const resolvedPlanModelId = resolveVectorProviderModelId(apiConfiguration.planModeClineModelId)
				const resolvedActModelId = resolveVectorProviderModelId(apiConfiguration.actModeClineModelId)
				const updates: Partial<GlobalStateAndSettings> = {}

				if (apiConfiguration.planModeClineModelId !== resolvedPlanModelId) {
					updates.planModeClineModelId = resolvedPlanModelId
				}
				if (apiConfiguration.actModeClineModelId !== resolvedActModelId) {
					updates.actModeClineModelId = resolvedActModelId
				}
				...
				if (planActSeparateModelsSetting) {
					...
					const modelId = modelIdField === "planModeClineModelId" ? resolvedPlanModelId : resolvedActModelId

					if (modelId && models[modelId]) {
						updates[modelInfoField] = models[modelId]
					}
				} else {
					if (resolvedPlanModelId && models[resolvedPlanModelId]) {
						updates.planModeClineModelInfo = models[resolvedPlanModelId]
					}
					if (resolvedActModelId && models[resolvedActModelId]) {
						updates.actModeClineModelInfo = models[resolvedActModelId]
					}
				}
