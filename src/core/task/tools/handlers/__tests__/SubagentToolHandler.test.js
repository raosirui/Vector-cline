...
function createConfig(options) {
    ...
    const config = {
        ...
        services: {
            stateManager: {
                getGlobalStateKey: (key) => (key === "nativeToolCallEnabled" ? true : undefined),
...
describe("SubagentToolHandler", () => {
    ...
    it("fans out prompts in parallel and emits aggregated status", async () => {
        ...
        let activeRuns = 0;
        let maxActiveRuns = 0;
        sinon.stub(SubagentRunner.prototype, "run").callsFake(async (_prompt, onProgress) => {
            activeRuns++;
        ...
        const usagePayload = JSON.parse(usageCalls[0].args[1]);
    ...
    it("continues after per-subagent failures and reports both outcomes", async () => {
        ...
        assert.equal(typeof result, "string");
        assert.ok(result.includes("Succeeded: 1"));
    ...
    it("runs configured subagent tools using the prompt parameter", async () => {
        ...
        sinon.stub(AgentConfigLoader, "getInstance").returns({
            resolveSubagentNameForTool: (toolName) => (toolName === dynamicToolName ? "code-reviewer" : undefined),
...
//# sourceMappingURL=SubagentToolHandler.test.js.map