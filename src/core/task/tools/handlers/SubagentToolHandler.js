...
const PROMPT_KEYS = ["prompt_1", "prompt_2", "prompt_3", "prompt_4", "prompt_5"];
function resolveConfiguredSubagentName(toolName) {
    return AgentConfigLoader.getInstance().resolveSubagentNameForTool(toolName);
}
function collectPrompts(block, configuredSubagentName) {
    ...
    return PROMPT_KEYS.map((key) => block.params[key]?.trim()).filter((prompt) => !!prompt);
}
function excerpt(text, maxChars = 1200) {
    if (!text) {
        return "";
    }
...
export class UseSubagentsToolHandler {
    ...
    async handlePartialBlock(block, uiHelpers) {
        ...
        const partialMessage = JSON.stringify({ prompts });
    ...
    async execute(config, block) {
        ...
        if (didAutoApprove) {
            ...
            showNotificationForApproval(prompts.length === 1
                ? `Cline wants to use ${configuredSubagentName ? `the '${configuredSubagentName}' subagent` : "a subagent"}`
                : `Cline wants to use ${prompts.length} subagents`, config.autoApprovalSettings.enableNotifications);
            const didApprove = await ToolResultUtils.askApprovalAndPushFeedback("use_subagents", approvalBody, config);
            if (!didApprove) {
                telemetryService.captureToolUsage(config.ulid, this.name, config.api.getModel().id, provider, false, false, undefined, block.isNativeToolCall);
                return formatResponse.toolDenied();
            }
            telemetryService.captureToolUsage(config.ulid, this.name, config.api.getModel().id, provider, false, true, undefined, block.isNativeToolCall);
        }
        ...
        const emitStatus = async (status, partial) => {
        ...
        const queueStatusUpdate = (status, partial) => {
        ...
        const execution = prompts.map((prompt, index) => runners[index].run(prompt, async (update) => {
        ...
        settled.forEach((result, index) => {
            if (result.status === "rejected") {
                ...
                entries[index].error = result.reason?.message || "Subagent execution failed";
                return;
            }
...
//# sourceMappingURL=SubagentToolHandler.js.map