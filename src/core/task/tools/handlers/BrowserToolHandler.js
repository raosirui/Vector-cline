    ...
    async handlePartialBlock(block, uiHelpers) {
        ...
        if (action === "launch") {
            if (uiHelpers.shouldAutoApproveTool(block.name)) {
                ...
                await uiHelpers.say("browser_action_launch", uiHelpers.removeClosingTag(block, "url", url), undefined, undefined, block.partial);
            }
            else {
            ...
            await uiHelpers.say(this.name, JSON.stringify({
                action: action,
        ...
        try {
            ...
            if (action === "launch") {
                ...
                if (autoApprover.shouldAutoApproveTool(block.name)) {
                    ...
                    showNotificationForApproval(`Cline wants to use a browser and launch ${url}`, config.autoApprovalSettings.enableNotifications);
                ...
                await config.callbacks.say(this.name, JSON.stringify({
                    action: action,
                    coordinate,
                    text,
