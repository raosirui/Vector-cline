...
export class UseSkillToolHandler {
    ...
    async execute(config, block) {
        ...
        try {
            ...
            telemetryService.safeCapture(() => telemetryService.captureSkillUsed({
                ulid: config.ulid,
                skillName,
...
IMPORTANT: The skill is now loaded. Do NOT call use_skill again for this task. Simply follow the instructions above to complete the user's request. You may access other files in the skill directory at: ${skillContent.path.replace(/SKILL\.md$/, "")}`;
            ...
            return `Error loading skill "${skillName}": ${error?.message}`;
        }
    }
}
//# sourceMappingURL=UseSkillToolHandler.js.map