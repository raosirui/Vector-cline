...
export class WebSearchToolHandler {
    ...
    async execute(config, block) {
        try {
            const query = block.params.query;
            const allowedDomainsRaw = block.params.allowed_domains;
            const blockedDomainsRaw = block.params.blocked_domains;
            ...
            if (config.callbacks.shouldAutoApproveTool(this.name)) {
                ...
                telemetryService.captureToolUsage(config.ulid, "web_search", config.api.getModel().id, provider, true, true, undefined, block.isNativeToolCall);
            }
            else {
                // Manual approval flow
                showNotificationForApproval(`Cline wants to search for: ${query}`, config.autoApprovalSettings.enableNotifications);
...
//# sourceMappingURL=WebSearchToolHandler.js.map