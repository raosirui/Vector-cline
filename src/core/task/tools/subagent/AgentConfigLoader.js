...
function normalizeToolName(toolName) {
    ...
    const asDefaultTool = trimmed;
    if (Object.values(ClineDefaultTool).includes(asDefaultTool)) {
        return asDefaultTool;
    }
    throw new Error(`Unknown tool '${trimmed}'. Expected a ClineDefaultTool value (for example: read_file, list_files, search_files).`);
}
...
export function parseAgentConfigFromYaml(content) {
...
export function getAgentsConfigPath(homeDir = os.homedir()) {
...
export async function readAgentConfigsFromDisk(homeDir = os.homedir()) {
    ...
    try {
        ...
        await Promise.all(yamlFiles.map(async (fileName) => {
...
export class AgentConfigLoader {
    ...
    cachedAgentToolNames = new Map();
    cachedToolNameToAgentName = new Map();
    listeners = new Set();
    constructor(homeDir = os.homedir()) {
        ...
        this.initialLoadPromise = this.load()
            ...
            .finally(() => this.watch().catch((error) => Logger.error("[AgentConfigLoader] Failed to start watching agent configs", error)));
    }
    static getInstance(homeDir = os.homedir()) {
     ...
     */
    static async resetInstanceForTests() {
        if (!AgentConfigLoader.instance) {
            return;
        }
    ...
    resolveSubagentNameForTool(toolName) {
        if (!toolName?.trim()) {
            return undefined;
        }
    ...
    isDynamicSubagentTool(toolName) {
        if (!toolName?.trim()) {
            return false;
        }
    ...
    rebuildDynamicToolMappings() {
        ...
        const agentToolNames = new Map();
        const toolNameToAgentName = new Map();
