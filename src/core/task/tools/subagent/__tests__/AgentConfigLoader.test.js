...
Prompt body`;
    ...
    it("loads all yaml/yml files from homeDir/.cline/data/agents", async () => {
        ...
        await fs.writeFile(path.join(directoryPath, "local-agent.yaml"), `---
name: local-agent
...
Prompt body`, "utf8");
        await fs.writeFile(path.join(directoryPath, "reviewer.yml"), `---
name: reviewer
...
Reviewer prompt`, "utf8");
    ...
    it("creates dynamic subagent tool mappings after loading configs", async () => {
        ...
        const directoryPath = getAgentsConfigPath(tempHome);
        await fs.mkdir(directoryPath, { recursive: true });
        await fs.writeFile(path.join(directoryPath, "code-reviewer.yaml"), `---
name: code reviewer
description: reviewer agent
tools: read_file
modelId: sonnet
...
//# sourceMappingURL=AgentConfigLoader.test.js.map