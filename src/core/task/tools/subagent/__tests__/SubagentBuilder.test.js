...
function createTaskConfig(mode, provider) {
    return {
        ulid: "ulid-123",
        services: {
            stateManager: {
                getGlobalSettingsKey: (key) => (key === "mode" ? mode : undefined),
                getApiConfiguration: () => ({
...
describe("SubagentBuilder", () => {
    ...
    it("uses cached config by subagent name and applies act-mode provider model override", () => {
        sinon.stub(AgentConfigLoader, "getInstance").returns({
            getCachedConfig: (subagentName) => subagentName === "cached-agent"
                ? {
                    name: "cached-agent",
        ...
        const buildApiHandlerStub = sinon.stub(api, "buildApiHandler").returns(fakeHandler);
        ...
        assert.equal(effectiveApiConfig.ulid, "ulid-123");
        assert.equal(effectiveApiConfig.actModeOpenAiModelId, "gpt-5");
        assert.equal(effectiveApiConfig.actModeApiModelId, "act-default");
    ...
    it("uses defaults when no cached config is provided", () => {
        ...
        sinon.stub(api, "buildApiHandler").returns({ getModel: sinon.stub(), createMessage: sinon.stub() });
    ...
    it("applies plan-mode openrouter model override fields", () => {
        sinon.stub(AgentConfigLoader, "getInstance").returns({
            getCachedConfig: (subagentName) => subagentName === "openrouter-agent"
                ? {
        ...
        assert.equal(effectiveApiConfig.planModeOpenRouterModelId, "openrouter/custom-model");
        assert.equal(effectiveApiConfig.planModeApiModelId, "plan-default");
        assert.equal(effectiveApiConfig.actModeApiModelId, "act-default");
    });
    it("builds native tools by filtering allowed ids and context requirements then converting", () => {
        sinon.stub(AgentConfigLoader, "getInstance").returns({
            getCachedConfig: (subagentName) => subagentName === "tools-agent"
                ? {
                    name: "tools-agent",
        ...
        sinon.stub(api, "buildApiHandler").returns({ getModel: sinon.stub(), createMessage: sinon.stub() });
        const getModelFamilyStub = sinon.stub(PromptRegistry.getInstance(), "getModelFamily").returns("test-family");
        ...
        const converter = sinon.stub().callsFake((tool) => ({ converted: tool.id }));
        const getConverterStub = sinon.stub(ClineToolSet, "getNativeConverter").returns(converter);
...
//# sourceMappingURL=SubagentBuilder.test.js.map