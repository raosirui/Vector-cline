...
function createTaskConfig(nativeToolCallEnabled) {
    return {
        taskId: "task-1",
        ulid: "ulid-1",
        cwd: "/tmp",
        ...
        services: {
            stateManager: {
                ...
                getGlobalStateKey: (key) => (key === "nativeToolCallEnabled" ? nativeToolCallEnabled : undefined),
                getApiConfiguration: () => ({
        ...
        coordinator: {
            getHandler: sinon.stub().callsFake((toolName) => {
...
function stubApiHandler(createMessage) {
...
describe("SubagentRunner", () => {
    ...
    it("emits native tool_use blocks with matching tool_result tool_use_id across turns", async () => {
        ...
        sinon.stub(promptRegistry, "get").callsFake(async () => {
            promptRegistry.nativeTools = [{ name: "list_files" }];
            return "system prompt";
        });
        sinon.stub(SubagentBuilder.prototype, "buildNativeTools").returns([{ name: "list_files" }]);
    ...
    it("passes prior request token totals into the next-turn compaction check", async () => {
        ...
        initializeHostProvider();
        const runner = new SubagentRunner(createTaskConfig(true));
        const shouldCompactStub = sinon.stub(runner, "shouldCompactBeforeNextRequest").callsFake((...args) => {
    ...
    it("falls back to non-native result blocks if structured tool calls appear while native mode is disabled", async () => {
        ...
        createMessage.onSecondCall().callsFake(async function* (_systemPrompt, conversation) {
            ...
            assert.equal(lastMessage.content.some((block) => block.type === "tool_result"), false);
            yield {
                type: "tool_calls",
                tool_call: {
                    function: {
    ...
    it("retries empty assistant turns with a no-tools-used nudge before failing", async () => {
        ...
        createMessage.onFirstCall().callsFake(async function* () { });
        createMessage.onSecondCall().callsFake(async function* (_systemPrompt, conversation) {
            const lastAssistant = conversation[1];
    ...
    it("retries initial stream failures before failing", async () => {
        ...
        createMessage.onFirstCall().callsFake(async function* () {
            yield* [];
            throw new Error('{"code":"stream_initialization_failed","message":"Failed to create stream: failed to generate stream from Vercel: failed to send request"}');
        });
        createMessage.onSecondCall().callsFake(async function* () {
            yield* [];
            throw new Error('{"code":"stream_initialization_failed","message":"Failed to create stream: failed to generate stream from Vercel: failed to send request"}');
    ...
    it("filters available skills to configured skills when subagent skills are configured", async () => {
        ...
        sinon.stub(promptRegistry, "get").callsFake(async (context) => {
            ...
            assert.deepEqual(context.skills.map((skill) => skill.name), ["allowed-skill"]);
            promptRegistry.nativeTools = undefined;
            return "system prompt";
        });
    ...
    it("uses all available skills when subagent skills are not configured", async () => {
        ...
        sinon.stub(promptRegistry, "get").callsFake(async (context) => {
            assert.ok(context.skills);
            assert.deepEqual(context.skills.map((skill) => skill.name), ["alpha-skill", "beta-skill"]);
            promptRegistry.nativeTools = undefined;
            return "system prompt";
        });
    ...
    it("logs a warning when a configured skill is not available", async () => {
        ...
        sinon.stub(promptRegistry, "get").callsFake(async (context) => {
            assert.ok(context.skills);
            assert.deepEqual(context.skills.map((skill) => skill.name), ["present-skill"]);
            promptRegistry.nativeTools = undefined;
            return "system prompt";
        });
    ...
    it("includes workspace metadata only in the initial user message", async () => {
        const createMessage = sinon.stub();
        createMessage.onFirstCall().callsFake(async function* (_systemPrompt, conversation) {
            const initialUser = conversation[0];
        ...
        createMessage.onSecondCall().callsFake(async function* (_systemPrompt, conversation) {
            const followUpUser = conversation[2];
...
//# sourceMappingURL=SubagentRunner.test.js.map