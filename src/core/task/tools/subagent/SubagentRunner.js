...
function createEmptyRequestUsageState() {
    return {
        inputTokens: 0,
        outputTokens: 0,
        cacheWriteTokens: 0,
...
function toToolUseParams(input) {
...
function formatToolArgPreview(value, maxLength = 48) {
...
function formatToolCallPreview(toolName, params) {
...
function resolveToolUseId(call, index) {
...
function toAssistantToolUseBlock(call) {
    return {
        type: "tool_use",
        id: call.toolUseId,
        name: call.name,
...
function parseNonNativeToolCalls(assistantText) {
...
function pushSubagentToolResultBlock(toolResultBlocks, call, label, content) {
...
export class SubagentRunner {
    ...
    abortingCommands = false;
    constructor(baseConfig, subagentName = "subagent") {
    ...
    async getWorkspaceMetadataEnvironmentBlock() {
        try {
            const workspacesJson = (await this.baseConfig.workspaceManager?.buildWorkspacesJson()) ??
                JSON.stringify({
                    workspaces: {
                        [this.baseConfig.cwd]: {
    ...
    async run(prompt, onProgress) {
        ...
        try {
            ...
            const nativeToolCallsRequested = providerInfo.model.info.apiFormat === ApiFormat.OPENAI_RESPONSES ||
            ...
            const skills = configuredSkillNames !== undefined
            ...
            while (true) {
                if (usageState.lastRequest &&
                    ...
                    const compactResult = this.compactConversationForContextWindow(contextManager, conversation, contextState.conversationHistoryDeletedRange);
                ...
                let requestId;
                const stream = this.createMessageWithInitialChunkRetry(api, systemPrompt, conversation, nativeTools, providerInfo.providerId, providerInfo.model.id, contextManager, contextState);
                ...
                const calculatedRequestCost = requestUsage.totalCost ??
                    calculateApiCostAnthropic(providerInfo.model.info, requestUsage.inputTokens, requestUsage.outputTokens, requestUsage.cacheWriteTokens, requestUsage.cacheReadTokens);
                requestUsage.totalTokens =
                    requestUsage.inputTokens +
                        requestUsage.outputTokens +
                        requestUsage.cacheWriteTokens +
                ...
                if (useNativeToolCalls) {
                    ...
                    Logger.warn("[SubagentRunner] Received structured tool_calls while native tool calling is disabled; falling back to non-native result serialization.");
                ...
                for (const call of finalizedToolCalls) {
                    ...
                    if (!handler) {
                        ...
                        try {
                            ...
                            toolResult = formatResponse.toolError(error.message);
                        }
                    }
                    stats.toolCalls += 1;
                    onProgress({ stats: { ...stats } });
            ...
            const errorText = error.message || "Subagent execution failed.";
    ...
    createSubagentTaskConfig(state) {
        ...
        return {
            ...
            callbacks: {
                ...baseCallbacks,
                say: async () => undefined,
                sayAndCreateMissingParamError: async (_toolName, paramName) => formatResponse.toolError(formatResponse.missingToolParameterError(paramName)),
                executeCommandTool: async (command, timeoutSeconds) => {
    ...
    shouldRetryInitialStreamError(error, providerId, modelId) {
    ...
    compactConversationForContextWindow(contextManager, conversation, conversationHistoryDeletedRange) {
    ...
    optimizeConversationForContextWindow(contextManager, conversation) {
        ...
        const optimizedConversation = optimizationResult.optimizedConversationHistory.map((message) => message);
    ...
    shouldCompactBeforeNextRequest(requestTotalTokens, api, modelId) {
    ...
    async *createMessageWithInitialChunkRetry(api, systemPrompt, fullConversation, nativeTools, providerId, modelId, contextManager, contextState) {
        for (let attempt = 1; attempt <= MAX_INITIAL_STREAM_ATTEMPTS; attempt += 1) {
            ...
            try {
                ...
                if (checkContextWindowExceededError(error)) {
                    const compactResult = this.compactConversationForContextWindow(contextManager, fullConversation, contextState.conversationHistoryDeletedRange);
                    contextState.conversationHistoryDeletedRange = compactResult.conversationHistoryDeletedRange;
                    if (!compactResult.didCompact || this.shouldAbort() || attempt >= MAX_INITIAL_STREAM_ATTEMPTS) {
                        throw error;
                    }
                    Logger.warn(`[SubagentRunner] Context window exceeded on initial stream attempt ${attempt}; compacted conversation and retrying.`);
                    continue;
                }
                const shouldRetry = !this.shouldAbort() &&
