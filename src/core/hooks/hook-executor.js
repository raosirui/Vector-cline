...
function fromHookOutput(output) {
...
export async function executeHook(options) {
    ...
    try {
        ...
        const streamCallback = async (line, stream, meta) => {
        ...
        const hook = await hookFactory.createWithStreaming(hookName, streamCallback, isCancellable ? abortController.signal : undefined, taskId, options.toolName);
