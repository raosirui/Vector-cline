...
class StdioHookRunner extends HookRunner {
    ...
    toolName;
    cwd;
    constructor(hookName, scriptPath, source, streamCallback, abortSignal, taskId, toolName, cwd) {
        super(hookName);
    ...
    async [exec](input) {
        ...
        if (taskId) {
            telemetryService.safeCapture(() => telemetryService.captureHookExecution(taskId, this.hookName, "started", {
                source: this.source,
            ...
            const parseJsonOutput = () => {
                try {
                    ...
                    if (output.contextModification && output.contextModification.length > MAX_CONTEXT_MODIFICATION_SIZE) {
                        Logger.warn(`Hook ${this.hookName} returned contextModification of ${output.contextModification.length} bytes, ` +
            ...
            if (parsedOutput) {
                ...
                if (taskId) {
                    if (parsedOutput.cancel) {
                        telemetryService.safeCapture(() => telemetryService.captureHookExecution(taskId, this.hookName, "completed", {
                            source: this.source,
                            toolName: this.toolName,
                            durationMs,
            ...
            if (HookExecutionError.isHookError(error)) {
                ...
                if (taskId) {
                    if (error.errorInfo.type === "cancellation") {
                        telemetryService.safeCapture(() => telemetryService.captureHookExecution(taskId, this.hookName, "cancelled", {
                            source: this.source,
                            toolName: this.toolName,
                        }), "HookFactory.exec.error.cancellation");
                    }
                    else if (error.errorInfo.type === "timeout") {
                        telemetryService.safeCapture(() => telemetryService.captureHookExecution(taskId, this.hookName, "failed", {
                            source: this.source,
                            toolName: this.toolName,
                            durationMs,
                            errorType: "timeout",
                        ...
                        telemetryService.safeCapture(() => telemetryService.captureHookExecution(taskId, this.hookName, "failed", {
                            ...
                            errorType: error.errorInfo.type,
                            errorMessage: error.message,
...
export class HookFactory {
     ...
     */
    async create(hookName, taskId, toolName) {
     ...
     */
    async createWithStreaming(hookName, streamCallback, abortSignal, taskId, toolName) {
        ...
        if (scripts.length > 0) {
            telemetryService.safeCapture(() => telemetryService.captureHookDiscovery(hookName, globalCount, workspaceCount), "HookFactory.createWithStreaming.discovery");
        }
     ...
     */
    determineHookCwd(scriptPath, hooksDirs, workspaceRoots, primaryCwd) {
        const containingDir = hooksDirs.find((dir) => scriptPath.startsWith(dir));
     ...
     */
    categorizeHookScripts(scripts, hooksDirs) {
        if (scripts.length === 0) {
    ...
    static async isHookFile(candidate, hookName) {
        try {
