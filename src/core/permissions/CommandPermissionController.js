...
const LINE_SEPARATOR_DESCRIPTIONS = {
...
export class CommandPermissionController {
     ...
     */
    validateParsedCommand(parsed, fullCommand) {
    ...
    parseCommandSegments(input) {
        ...
        function process(tokenList) {
            ...
            for (let i = 0; i < tokenList.length; i++) {
                ...
                if (typeof token === "object" && "op" in token && COMMAND_SEPARATOR_OPERATORS.has(token.op)) {
                    flushSegment();
                    continue;
                }
                // 3. Handle Redirect Operators: >, >>, <, etc.
                if (typeof token === "object" && "op" in token && REDIRECT_OPERATORS.has(token.op)) {
     ...
     */
    detectDangerousCharsOutsideQuotes(command) {
...
//# sourceMappingURL=CommandPermissionController.js.map