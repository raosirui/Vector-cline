...
import { condenseToolResponse, deepPlanningToolResponse, explainChangesToolResponse, newRuleToolResponse, newTaskToolResponse, reportBugToolResponse, } from "../prompts/commands";
import { StateManager } from "../storage/StateManager";
/**
 * Processes text for slash commands and transforms them with appropriate instructions
 * This is called after parseMentions() to process any slash commands in the user's message
 */
export async function parseSlashCommands(text, localWorkflowToggles, globalWorkflowToggles, ulid, focusChainSettings, enableNativeToolCalls, providerInfo, mcpPromptFetcher) {
                ...
                if (parts.length >= 3) {
                    ...
                    try {
                        ...
                        if (promptResponse) {
                            ...
                            const textWithoutSlashCommand = removeSlashCommand(text, tagContent, contentStartIndex, slashMatch);
                            const processedText = `<mcp_prompt server="${serverName}" prompt="${promptName}">\n${promptContent}\n</mcp_prompt>\n` +
 ...
 */
export function formatMcpPromptResponse(response) {
    const parts = [];
