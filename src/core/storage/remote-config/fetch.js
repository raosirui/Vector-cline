...
async function fetchRemoteConfigForOrganization(organizationId) {
    try {
        ...
        const configData = await makeAuthenticatedRequest(CLINE_API_ENDPOINT.REMOTE_CONFIG, organizationId);
...
async function fetchApiKeysForOrganization(organizationId) {
    try {
        ...
        const response = await makeAuthenticatedRequest(CLINE_API_ENDPOINT.API_KEYS, organizationId);
