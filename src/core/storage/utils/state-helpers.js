import { applyTransform, GlobalStateAndSettingKeys, getDefaultValue, isAsyncProperty, isComputedProperty, LocalStateKeys, SecretKeys, } from "@shared/storage/state-keys";
 ...
 */
export function readSecretsFromStorage(store) {
 ...
 */
export function readWorkspaceStateFromStorage(store) {
 ...
 */
export async function readGlobalStateFromStorage(store) {
    try {
 ...
 */
async function handleComputedProperties(result, stateValues) {
    // 1. API Provider logic - set defaults based on existing values
    const defaultApiProvider = "openrouter";
 ...
 */
async function handleAsyncProperties(result) {
